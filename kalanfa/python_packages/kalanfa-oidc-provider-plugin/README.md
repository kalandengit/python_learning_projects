# Kalanfa OpenID Connect Provider plugin
...
This package provides a plugin to convert a Kalanfa server into an OIDC provider that can be used by external applications to authenticate, thus Kalanfa becomes the source of truth when integrating it with other applications needing a single-sign-on (SSO) authentication.

## How can I install this plugin?

1. Inside your Kalanfa virtual environment: `pip install kalanfa-oidc-provider-plugin`

2. Activate the plugin: `kalanfa plugin enable kalanfa_oidc_provider_plugin`
...
2. Activate the plugin:
   ...
   `KALANFA_HOME="$(pwd)/.kalanfa" uv run kalanfa plugin enable kalanfa_oidc_provider_plugin`
...
`kalanfa manage creatersakey`

## Creating new authorization clients
This plugin adds a management command, `oidccreateclient`, to create new clients that can use Kalanfa to authenticate and authorize their users.
...
Usage examples:
`kalanfa manage oidccreateclient --name=myapp --clientid=myclient.app --redirect-uri="http://localhost:9000/openidconnect/api/callback/"`

or, if the site needs to use different redirect uris in the client server:

```
kalanfa manage oidccreateclient --name=myapp --clientid=myclient.app --redirect-uri="http://localhost:9000/oidc/callback/
...
All the server endpoints are available via the `OpenID Provider Discovery` url in the server provider address http://server/.well-known/openid-configuration , for example, if running Kalanfa locally, at http://localhost:8080/.well-known/openid-configuration
...
```
Or supply the `REQUIRE_CONSENT` option setting in an environment variable called `KALANFA_OIDC_PROVIDER_REQUEST_CONSENT`.
