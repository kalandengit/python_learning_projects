...
from kalanfa.core.auth.errors import InvalidRoleKind

logger = logging.getLogger(__name__)


class OIDCKalanfaAuthenticationBackend(OIDCAuthenticationBackend):
    ...
    def create_user(self, claims):
        ...
        # not needed in Kalanfa, email is not mandatory:
        email = claims.get("email", username)
        # Kalanfa doesn't allow an empty password. This isn't going to be used:
        password = uuid4().hex
        ...
        for role in roles:
            if role.lower() in ("admin", "coach"):
                try:
                    ...
                    pass  # The role does not exist in Kalanfa
