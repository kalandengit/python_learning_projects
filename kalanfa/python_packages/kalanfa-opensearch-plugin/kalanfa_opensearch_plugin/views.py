...
from kalanfa.core.content.api import ContentNodeSearchViewset
...
class Descriptor(View):
    def get(self, request):
        ...
        xml = (
            ...
            "<ShortName>Kalanfa</ShortName>"
            "<Description>Kalanfa Open Search Engine</Description>"
            + (
...
class Search(View):
    def get(self, request):
        ...
        feed = feedgenerator.Atom1Feed(
            title="Kalanfa search results",
            link=request.build_absolute_uri(),
            description="Kalanfa search results for query {value}".format(value=value),
        )

        for result in results:
            node_link = request.build_absolute_uri(
                "{url}?node_id={id}&channel_id={channel_id}&content_id={content_id}".format(
                    url=reverse("kalanfa:core:contentpermalink"),
                    id=result.id,
