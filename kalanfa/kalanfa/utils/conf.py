...
KALANFA_HOME = os.path.abspath(os.path.expanduser(os.environ["KALANFA_HOME"]))
NO_FILE_BASED_LOGGING = os.environ.get("KALANFA_NO_FILE_BASED_LOGGING", False)

# Creating KALANFA_HOME atm. has to happen here as for instance utils.cli is not
# called through py.test. This file is the first basic entry point of
# Kalanfa, although utils.cli may or may not precede it.
if not os.path.exists(KALANFA_HOME):
    parent = os.path.dirname(KALANFA_HOME)
    if not os.path.exists(parent):
        raise RuntimeError(
            "The parent of your KALANFA_HOME does not exist: {}".format(parent)
        )
    os.mkdir(KALANFA_HOME)

# Create a folder named logs inside KALANFA_HOME to store all the log files.
LOG_ROOT = os.path.join(KALANFA_HOME, "logs")
