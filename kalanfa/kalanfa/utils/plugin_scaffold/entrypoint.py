...
``kalanfa.plugins`` entry point must be inserted into the nearest
...
ENTRY_POINT_HEADER = '[project.entry-points."kalanfa.plugins"]'
...
def module_import_path(module_dir, pyproject_dir):
    ...
    ``<pp>/kalanfa/plugins/my_thing`` -> ``"kalanfa.plugins.my_thing"``.
    """
...
def insert_entry_point(pyproject_path, name):
    """
    Insert a ``kalanfa.plugins`` entry point into ``pyproject_path``.

    The line ``"<name>" = "<name>"`` is placed at its sorted position
    within the ``[project.entry-points."kalanfa.plugins"]`` table, without
