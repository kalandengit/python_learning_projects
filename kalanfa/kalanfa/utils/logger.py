...
NO_FILE_BASED_LOGGING = os.environ.get("KALANFA_NO_FILE_BASED_LOGGING", False)
DISABLE_REQUEST_LOGGING = os.environ.get("KALANFA_DISABLE_REQUEST_LOGGING", False)
...
class KalanfaTimedRotatingFileHandler(TimedRotatingFileHandler):
    """
    A custom TimedRotatingFileHandler that overrides two methods, getFilesToDelete
    and doRollover, to rename the rotation files from KALANFA_HOME/logs/kalanfa.txt.YYYY-MM-DD
    to KALANFA_HOME/logs/archive/KALANFA-YYYY-MM-DD.txt.
    ...
    def getFilesToDelete(self):
        ...
        # Find all the rotation files in the KALANFA_HOME/logs/archive directory
    ...
    def doRollover(self):
        ...
        be renamed from KALANFA_HOME/logs/kalanfa.txt.YYYY-MM-DD to
        KALANFA_HOME/logs/archive/KALANFA-YYYY-MM-DD.txt.
        """
        super().doRollover()
        filenames = os.listdir(self.dirname)
        prefix = self.basename + "."

        # Find all the rotation files in the KALANFA_HOME/logs directory and rename
        # them.
        if not os.path.exists(self.archive_dir):
...
def get_default_logging_config(LOG_ROOT, debug=False, debug_database=False):
    """
    A minimal logging config for just kalanfa without any Django
    specific handlers or anything from kalanfa.utils.conf.
    ...
    handlers = {
        "console-error": {
            "level": "ERROR",
            "class": "kalanfa.utils.logger.EncodingStreamHandler",
            "formatter": "color",
    ...
    if not NO_FILE_BASED_LOGGING:
        handlers["file"] = {
            "level": "INFO",
            "filters": [],
            "class": "kalanfa.utils.logger.KalanfaTimedRotatingFileHandler",
            "filename": os.path.join(LOG_ROOT, "kalanfa.txt"),
...
def get_base_logging_config(LOG_ROOT, debug=False, debug_database=False):
    ...
    kalanfa.utils.conf and kalanfa.utils.options need logging, too and
...
def get_logging_config(LOG_ROOT, debug=False, debug_database=False):
    ...
    for name in ("kalanfa", "django", "django.request"):
