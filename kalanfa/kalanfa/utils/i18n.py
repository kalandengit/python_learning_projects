...
def _get_language_info():
    file_path = os.path.abspath(
        os.path.join(os.path.dirname(kalanfa.__file__), "locale", "language_info.json")
    )
...
# Kalanfa format
KALANFA_LANGUAGE_INFO = _get_language_info()

# List of intl codes that Kalanfa officially supports
KALANFA_SUPPORTED_LANGUAGES = {
    "ar",
    "bg-bg",
    "bn-bd",
    "de",
    ...
    for loc in (locale.getlocale()[0], locale.getdefaultlocale()[0]):
        if loc:
            ...
            for lang_code in (lang, lang.split("-")[0]):
                if lang_code in KALANFA_SUPPORTED_LANGUAGES:
                    return lang_code
