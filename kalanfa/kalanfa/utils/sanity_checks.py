...
def check_content_directory_exists_and_writable():
    """
    Make sure the content directory of Kalanfa exists and is writable.
    """
...
def check_log_file_location():
    """
    Starting from Kalanfa v0.12.4, log files are going to be renamed and moved
    from KALANFA_HOME directory to KALANFA_HOME/logs directory.
    """
    home = os.environ["KALANFA_HOME"]
    log_location_update = {}

    # Old log file names
    old_daemon_log = "server.log"
    old_kalanfa_log = "kalanfa.log"
    old_debug_log = "debug.log"

    # New log file names
    log_location_update[old_daemon_log] = "daemon.txt"
    log_location_update[old_kalanfa_log] = "kalanfa.txt"
...
def check_database_is_migrated():
    ...
    assumed to be a part of Kalanfa version number checking. When a
    Kalanfa version change is detected, we run migrations. Checking that
...
def check_default_options_exist():
    options_path = os.path.join(KALANFA_HOME, "options.ini")
