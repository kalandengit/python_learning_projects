...
from kalanfa.utils.logger import cleanup_queue_logging
from kalanfa.utils.logger import setup_queue_logging
from kalanfa.utils.server.hooks import KalanfaProcessHook
from kalanfa.utils.system import become_daemon
from kalanfa.utils.system import pid_exists

logger = logging.getLogger(__name__)

# Status codes for kalanfa
STATUS_RUNNING = 0
STATUS_STOPPED = 1
...
PID_FILE = os.path.join(conf.KALANFA_HOME, "server.pid")

# File used to activate profiling middleware and get profiler PID
PROFILE_LOCK = os.path.join(conf.KALANFA_HOME, "server_profile.lock")

# File used to send a state transition command to the server process
PROCESS_CONTROL_FLAG = os.path.join(conf.KALANFA_HOME, "process_control.flag")
...
status_messages = {
    ...
    STATUS_UNKNOWN_INSTANCE: "Unknown Kalanfa running on port",
    STATUS_SERVER_CONFIGURATION_ERROR: "Kalanfa server configuration error",
...
class KalanfaServerPlugin(ServerPlugin):
    def __init__(self, bus, port):
        ...
        if (
            ...
            logger.error(
                "There is another Kalanfa server running. "
                "Please use `kalanfa stop` and try again."
            )
            raise RunningException("There is another Kalanfa server running.")
        super().__init__(bus, port)

    @property
    def application(self):
        from kalanfa.deployment.default.wsgi import application
    ...
    def START(self):
        ...
        for url in urls:
            self.bus.publish("log", "Kalanfa running on: {}".format(url), 20)
...
class ZipContentServerPlugin(ServerPlugin):
    @property
    def application(self):
        from kalanfa.deployment.default.alt_wsgi import alt_application
...
class DefaultScheduledTasksPlugin(SimplePlugin):
    def START(self):
        from kalanfa.core.analytics.tasks import schedule_local_notification_generation
        from kalanfa.core.analytics.tasks import schedule_ping
        from kalanfa.core.deviceadmin.tasks import schedule_streamed_cache_cleanup
        from kalanfa.core.deviceadmin.tasks import schedule_vacuum
...
class ServicesPlugin(SimplePlugin):
    ...
    def START(self):
        from kalanfa.core.tasks.main import initialize_workers
...
class ZeroConfPlugin(Monitor):
    ...
    def RUN(self):
        # Register the Kalanfa zeroconf service so it will be discoverable on the network
        from kalanfa.core.discovery.utils.network.broadcast import (
            build_broadcast_instance,
        )
        from kalanfa.core.discovery.utils.network.broadcast import KalanfaBroadcast
        from kalanfa.core.discovery.utils.network.local_hostnames import (
            LocalHostnameListener,
        )
        from kalanfa.core.discovery.utils.network.search import NetworkLocationListener

        instance = build_broadcast_instance(self.port)

        if self.broadcast is None:
            self.broadcast = KalanfaBroadcast(instance, interfaces=self.interfaces)
    ...
    def run(self):
        ...
        # zeroconf, the listener, and the broadcast kalanfa service.
...
def get_local_hostnames():
    """
    Returns the `.local` hostnames (e.g. ["kalanfa.local", "tonyslaptop.local"])
...
class PIDPlugin(SimplePlugin):
    ...
    def set_pid_file(self, status, *args):
        """
        Writes a PID file in the format Kalanfa parses
...
class SystemdNotifyPlugin(SimplePlugin):
    ...
    Kalanfa process plugins have started successfully. In particular, zeroconf
...
def _port_check(port):
    # In case that something other than Kalanfa occupies the port,
...
class DaemonizePlugin(SimplePlugin):
    ...
    def ENTER(self):
        self.bus.publish("log", "Running Kalanfa as background process", 20)
        if self.check_ports:
            ...
            if self.bus.port:
                ...
                for url in urls:
                    logger.info("Kalanfa running on: {}".format(url))
            else:
                logger.info(
                    "No port specified, for information about accessing the server, run kalanfa status"
                )
        # Daemonize at this point, no more user output is needed

        kalanfa_log = settings.LOGGING["handlers"]["file"]["filename"]
        self.bus.publish(
            "log", "Going to background mode, logging to {0}".format(kalanfa_log), 20
        )
...
def stop():
    ...
    if pid_exists(pid):
        ...
        try:
            ...
            logger.debug(
                "Received an error while trying to kill the Kalanfa process: %s" % e
            )
    starttime = time.time()
    while time.time() - starttime <= 10:
        if pid_exists(pid):
            ...
            break
    if pid_exists(pid):
        logging.error("Kalanfa process has failed to shutdown")
        return STATUS_UNCLEAN_SHUTDOWN
    return STATUS_STOPPED


class BaseKalanfaProcessBus(ProcessBus):
    ...
    def __init__(
        ...
        logger.info("Starting Kalanfa {version}".format(version=kalanfa.__version__))
...
class KalanfaServicesProcessBus(BaseKalanfaProcessBus):
...
class KalanfaProcessBus(KalanfaServicesProcessBus):
    """
    This class is the state machine that manages the starting, restarting, and shutdown of
    a running Kalanfa instance. It is responsible for starting any WSGI servers that respond
    to HTTP requests in the Kalanfa lifecycle, and also other ancillary services like
    a ZeroConf server, task runner work pool, etc.

    The primary use case for this process bus is for running Kalanfa in a consumer server or
    ...
    def __init__(self, *args, **kwargs):
        ...
        for process_hook in KalanfaProcessHook.registered_hooks:
            process_plugin = process_hook.MagicBusPluginClass(self)
            process_plugin.subscribe()

        kalanfa_server = KalanfaServerPlugin(
            self,
            self.port,
        )
...
def start(port=0, zip_port=0, serve_http=True, background=False):
    ...
    """
    if serve_http:
        bus = KalanfaProcessBus(
            port=port,
            zip_port=zip_port,
        )
    else:
        bus = KalanfaServicesProcessBus(
            port=port,
            zip_port=zip_port,
        )

    # On Mac, Python crashes when forking the process, so prevent daemonization until we can figure out
    # a better fix. See https://github.com/learningequality/kalanfa/issues/4821
...
def get_status():  # noqa: C901
    ...
    The behavior is also quite redundant given that `kalanfa start` should
    ...
    if conf.OPTIONS["Server"]["CHERRYPY_START"]:
        try:
            ...
            # TODO: Using 127.0.0.1 is a hardcode default from Kalanfa, it could
            # be configurable
        ...
        if response.status_code != 200:
            # Probably a mis-configured kalanfa
    ...
    # is dumb because kalanfa could be running in a worker pool with different
...
def get_installer_version(installer_type):  # noqa: C901
    ...
    def get_deb_kalanfaserver_version():
        return get_debian_pkg_version("kalanfa-server")

    def get_deb_version():
        return get_debian_pkg_version("kalanfa")

    def get_apk_version():
        return os.environ.get("KALANFA_APK_VERSION_NAME")

    installer_version = os.environ.get("KALANFA_INSTALLER_VERSION")
    if installer_version:
        return installer_version

    version_funcs = {
        installation_types.DEB: get_deb_version,
        installation_types.KALANFA_SERVER: get_deb_kalanfaserver_version,
...
def installation_type(cmd_line=None):  # noqa:C901
    """
    Tries to guess how the running kalanfa server was installed

    :returns: install_type is the type of detected installation
    """

    install_type = os.environ.get("KALANFA_INSTALLATION_TYPE", "Unknown")
    ...
    def is_debian_package():
        ...
        try:
            check_output(["dpkg", "-s", "kalanfa"])
        except (
            CalledProcessError,
            FileNotFoundError,
        ):  # kalanfa package not installed!
            if sys.path[-1] != "/usr/lib/python3/dist-packages":
                install_type = installation_types.WHL
        return install_type

    def is_kalanfa_server():
        # running under uwsgi, finding out if we are using kalanfa-server
        install_type = "Unknown"
        try:
            check_output(["dpkg", "-s", "kalanfa-server"])
            install_type = installation_types.KALANFA_SERVER
        except CalledProcessError:  # kalanfa-server package not installed!
            install_type = installation_types.WHL
        return install_type

    # in case the KALANFA_INSTALLATION_TYPE is not set, let's use the old method:
    if install_type == "Unknown":
        if on_android():
            install_type = installation_types.APK
        elif os.environ.get("KALANFA_DEVELOPER_MODE", False):
            ...
            elif launcher == "/usr/bin/kalanfa":
                install_type = is_debian_package()
            elif launcher == "uwsgi":
                package = is_debian_package()
                if package != "whl":
                    install_type = is_kalanfa_server()
            elif "\\Scripts\\kalanfa" in launcher:
                paths = sys.path
                for path in paths:
                    if "kalanfa.exe" in path:
