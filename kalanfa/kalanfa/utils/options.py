"""
This module is intended to allow customization of Kalanfa settings with the
options.ini file.
...
from kalanfa.plugins.utils.options import extend_config_spec
from kalanfa.utils.data import bytes_from_humans
from kalanfa.utils.i18n import KALANFA_LANGUAGE_INFO
from kalanfa.utils.i18n import KALANFA_SUPPORTED_LANGUAGES
from kalanfa.utils.system import get_fd_limit
...
ALL_LANGUAGES = "kalanfa-all"
SUPPORTED_LANGUAGES = "kalanfa-supported"
...
def _process_language_string(value):
    ...
    if value == ALL_LANGUAGES:
        return list(KALANFA_LANGUAGE_INFO.keys())
    if value == SUPPORTED_LANGUAGES:
        return list(KALANFA_SUPPORTED_LANGUAGES)
    if value in KALANFA_LANGUAGE_INFO:
        return [value]
    raise ValueError
...
def language_list(value):
    ...
    String can be any value that is a key of KALANFA_LANGUAGE_INFO
...
def path(value):
    ...
    if value:
        # ensure all path arguments, e.g. under section "Paths", are fully resolved and expanded, relative to KALANFA_HOME
        return os.path.join(KALANFA_HOME, os.path.expanduser(value))
    return value
...
def cache_option(value):
    ...
    try:
        if value == "redis":
            ...
            from kalanfa.core.utils.cache import RedisCache  # noqa
            import redis  # noqa
        return value
    except ImportError:
        logger.error(
...
def lazy_import_callback(value):
    ...
    is internal to Kalanfa, and also because the module may not be available
    in some contexts.
    """
    if not isinstance(value, str):
        raise VdtValueError(value)
...
base_option_spec = {
    ...
    "Database": {
        ...
        "DATABASE_NAME": {
            "type": "string",
            "description": """
                For SQLite - the name of a database file to use for the main Kalanfa database.
                For Postgresql, the name of the database to use for all Kalanfa data.
            """,
        },
        "DATABASE_PASSWORD": {
            "type": "string",
    ...
    "Server": {
        "CHERRYPY_START": {
            "type": "boolean",
            "default": True,
            "description": "DEPRECATED - do not use this option, use the 'kalanfa services' command instead.",
            "deprecated": True,
        },
        "CHERRYPY_THREAD_POOL": {
            "type": "integer",
            "default": calculate_thread_pool(),
            "description": "How many threads the Kalanfa server should use to serve requests",
        },
        ...
        "PROFILE": {
            "type": "boolean",
            "default": False,
            "envvars": ("KALANFA_SERVER_PROFILE",),
            "description": "Activate the server profiling middleware.",
        },
        "DEBUG": {
            "type": "boolean",
            "default": False,
            "description": "Run Kalanfa with Django setting DEBUG = True",
        },
        "DEBUG_LOG_DATABASE": {
            "type": "boolean",
            "default": False,
    ...
    "Paths": {
        ...
        "CONTENT_FALLBACK_DIRS": {
            "type": "path_list",
            "default": "",
            "description": "Additional directories in which Kalanfa will look for content files and content database files.",
        },
    ...
    "Urls": {
        "CENTRAL_CONTENT_BASE_URL": {
            ...
            "description": """
                ...
                Slightly counterintuitively this will still be displayed in the UI as 'import from Kalanfa Studio'.
            """,
        },
        "DATA_PORTAL_SYNCING_BASE_URL": {
            "type": "string",
            "default": "https://kalanfadataportal.learningequality.org",
    ...
    "Deployment": {
        "HTTP_PORT": {
            "type": "port",
            "default": 8080,
            "deprecated_envvars": ("KALANFA_LISTEN_PORT",),
            "description": "Sets the port that Kalanfa will serve on. This can be further overridden by command line arguments.",
        },
        "RUN_MODE": {
            "type": "string",
            "description": "Used to flag non-user Kalanfa instances",
            "skip_blank": True,
        },
        "DISABLE_PING": {
            "type": "boolean",
        ...
        "URL_PATH_PREFIX": {
            ...
            "default": "/",
            "description": """
                Serve Kalanfa from a subpath under the main domain. Used when serving multiple applications from
        ...
        "LANGUAGES": {
            ...
            "description": """
                The user interface languages to enable on this instance of Kalanfa (has no effect on languages of imported content channels).
                The default will include all the languages Kalanfa supports.
            """,
        },
        "ZIP_CONTENT_ORIGIN": {
            "type": "origin_or_port",
            "default": "",
            "description": """
                When running by default (value blank), Kalanfa frontend looks for the zipcontent endpoints
                on the same domain as Kalanfa proper, but uses ZIP_CONTENT_PORT instead of HTTP_PORT.
                When running behind a proxy, set the value to the port where zipcontent endpoint is served on,
                and it will be substituted for the port that Kalanfa proper is being served on.
                ...
                It is strongly recommended that zipcontent is served from a different origin from the main Kalanfa app,
        ...
        "ZIP_CONTENT_PORT": {
            ...
            "default": 0,
            "description": """
                Sets the port that Kalanfa will serve the alternate origin server on. This is the server that
        ...
        "ZIP_CONTENT_URL_PATH_PREFIX": {
            ...
            "description": """
                ...
                origin server, and the URL prefix where the Kalanfa frontend looks for it.
                In the case that ZIP_CONTENT_ORIGIN is pointing to an entirely separate origin, this setting
                can still be used to set a URL prefix that the frontend of Kalanfa will look to when
        ...
        "MINIMUM_DISK_SPACE": {
            ...
            "default": "250MB",
            "description": """
                The minimum free disk space that Kalanfa should try to maintain on the device. This will
                be used as the floor value to prevent Kalanfa completely filling the disk during file import.
                Value can either be a number suffixed with a unit (e.g. MB, GB, TB) or an integer number of bytes.
            """,
        },
        "LISTEN_ADDRESS": {
        ...
        "RESTART_HOOKS": {
            ...
            "default": ["kalanfa.utils.server.signal_restart"],
            "description": """
        ...
        "CSP_HOST_SOURCES": {
            ...
            "description": """
                ...
                Allowing deployed Kalanfa servers to specify additional hosts from which content can be loaded.
            """,
        },
    },
    "Tasks": {
    ...
    for section, opts in option_spec.items():
        for optname, attrs in opts.items():
            ...
            if default_envvar not in envvars:
                ...
                default_envvar = "KALANFA_{}_{}".format(
...
def read_options_file(ini_filename="options.ini"):
    ...
    ini_path = os.path.join(KALANFA_HOME, ini_filename)
...
def generate_empty_options_file(ini_filename="options.ini"):
    # Generate an options.ini file inside the KALANFA_HOME as default placeholder config
