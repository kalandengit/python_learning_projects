"""
Integration tests for the ``kalanfa plugin create`` scaffolder
(``kalanfa.utils.plugin_scaffold``).
...
``kalanfa_plugin.py`` imports and exposes the right hooks, that its entry point
is registered, and (in module mode) that it is discoverable and enable-able
through Kalanfa's real plugin registry — across both modes and every surface.
"""
...
from kalanfa.core.content.hooks import ContentRendererHook
...
from kalanfa.utils.plugin_scaffold import CONTENT_VIEWER
from kalanfa.utils.plugin_scaffold import GLOBAL_INJECTOR
from kalanfa.utils.plugin_scaffold import MODE_MODULE
from kalanfa.utils.plugin_scaffold import MODE_PACKAGE
from kalanfa.utils.plugin_scaffold import scaffold_plugin
from kalanfa.utils.plugin_scaffold import SINGLE_PAGE_APP
...
def _load_kalanfa_plugin(path, unique):
    """
    Import a generated ``kalanfa_plugin.py`` under a unique module name.

    ``register_hook`` requires the defining module's name to end with
    ``kalanfa_plugin``, so the unique prefix is suffixed accordingly.
    """
    name = "{}_kalanfa_plugin".format(unique)
...
def _assert_surface_plugin(module, surface):
    """Assert the imported ``kalanfa_plugin`` module matches the surface."""
    assert _hook_subclasses(module, KalanfaPluginBase), "no KalanfaPluginBase subclass"
...
def test_package_mode_plugin_imports_and_registers_entry_point(tmp_path, surface):
    ...
    outer = tmp_path / "kalanfa-my-thing-plugin"
    pkg = outer / "kalanfa_my_thing_plugin"
    assert (pkg / "__init__.py").is_file()

    # Package mode declares its own entry point in its own pyproject.toml.
    assert result.entry_point_name == "kalanfa_my_thing_plugin"
    assert result.registration_note is None
    assert (
        'kalanfa_my_thing_plugin = "kalanfa_my_thing_plugin"'
        in (outer / "pyproject.toml").read_text()
    )

    module = _load_kalanfa_plugin(
        str(pkg / "kalanfa_plugin.py"), "pkg_" + surface.replace("-", "_")
    )
    ...
    if surface == BACKEND_ONLY:
        ...
        # package.json lives at the package root (like kalanfa-sentry-plugin),
...
def test_module_mode_plugin_discoverable_and_enableable(tmp_path, plugins, surface):
    ...
    (tmp_path / "pyproject.toml").write_text(
        '[project.entry-points."kalanfa.plugins"]\n"aaa" = "aaa"\n"zzz" = "zzz"\n'
    )
    ...
    try:
        # Discoverable: the registry loads the generated kalanfa_plugin.py.
        plugin = initialize_kalanfa_plugin(import_path, initialize_hooks=False)
        assert isinstance(plugin, KalanfaPluginBase)
        _assert_surface_plugin(
            importlib.import_module(import_path + ".kalanfa_plugin"), surface
        )
...
def test_module_mode_without_enclosing_pyproject_returns_registration_note(tmp_path):
    ...
    assert (tmp_path / "my_thing" / "kalanfa_plugin.py").is_file()
...
def test_single_page_app_frontend_and_api_files(tmp_path):
    ...
    pkg = tmp_path / "kalanfa-my-thing-plugin" / "kalanfa_my_thing_plugin"
    for rel in (
        "api_urls.py",
        "urls.py",
        "viewsets.py",
    ...
    assert "{% webpack_asset 'kalanfa_my_thing_plugin.app' %}" in html
    ...
    # reverse of ``kalanfa:<module>:my_thing`` resolves to the API endpoint
    # instead of the page.
    ...
    assert "urls['kalanfa:kalanfa_my_thing_plugin:my_thing']" in side_nav
...
def test_package_json_uses_workspace_and_catalog_inside_a_workspace(tmp_path):
    # A workspace providing kalanfa/kalanfa-viewer as members and
    # kalanfa-design-system/vue in the catalog.
    _write_pnpm_workspace(
        tmp_path,
        catalog=("kalanfa-design-system", "vue"),
        member_names=("kalanfa", "kalanfa-common", "kalanfa-viewer"),
    )
    target = tmp_path / "plugins"
    target.mkdir()

    _scaffold(target, MODE_PACKAGE, CONTENT_VIEWER)
    outer = target / "kalanfa-my-thing-plugin"
    deps = json.loads((outer / "package.json").read_text())["dependencies"]
    # Member packages -> workspace:*, catalog entries -> catalog:.
    assert deps["kalanfa"] == "workspace:*"
    assert deps["kalanfa-viewer"] == "workspace:*"
    assert deps["kalanfa-design-system"] == "catalog:"
...
def test_package_json_falls_back_to_pins_outside_a_workspace(tmp_path):
    ...
    outer = tmp_path / "kalanfa-my-thing-plugin"
    deps = json.loads((outer / "package.json").read_text())["dependencies"]
    assert deps["kalanfa"] == "^0.18.0"
    assert deps["kalanfa-viewer"] == "^1.0.0"
    assert deps["kalanfa-design-system"] == "^5.7.0"
