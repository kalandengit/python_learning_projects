"""
Tests for `kalanfa.utils.main` module.
"""
...
import kalanfa
from kalanfa.utils import main
...
@patch("kalanfa.utils.main._upgrades_after_django_setup")
@patch("kalanfa.utils.main.get_version", return_value="")
@patch("kalanfa.utils.main.update")
@patch("kalanfa.core.deviceadmin.utils.dbbackup")
def test_first_run(
...
@patch("kalanfa.plugins.registry.is_initialized", return_value=False)
@patch("kalanfa.utils.main._upgrades_after_django_setup")
@patch("kalanfa.utils.main.get_version", return_value="0.0.1")
@patch("kalanfa.utils.main.update")
...
def test_update_exits_if_running(get_version, is_initialized):
    ...
    Tests that update() function performs as expected
    """
    with patch("kalanfa.utils.main.get_status"):
        try:
            main.initialize()
            pytest.fail("Update did not exit when Kalanfa was already running")
        except SystemExit:
            pass
...
def test_conditional_backup():
    ...
    version = truncate_version(kalanfa.__version__)
...
@patch("kalanfa.plugins.registry.is_initialized", return_value=False)
@patch("kalanfa.utils.main._upgrades_after_django_setup")
@patch("kalanfa.utils.main.get_version", return_value=kalanfa.__version__)
@patch("kalanfa.utils.main.update")
@patch("kalanfa.core.deviceadmin.utils.dbbackup")
...
@patch("kalanfa.utils.main.shutil.copyfile")
@patch("kalanfa.utils.main.os.path.exists", return_value=False)
def test_copy_preseeded_db_uses_target_basename(mock_exists, mock_copyfile):
    ...
    with patch.dict("sys.modules", {"kalanfa.dist": mock_dist}):
        ...
        mock_copyfile.assert_called_once_with(
            expected_source,
            os.path.join(KALANFA_HOME, "db.sqlite3"),
        )


@patch("kalanfa.plugins.registry.is_initialized", return_value=False)
@patch("kalanfa.utils.main._upgrades_after_django_setup")
@patch("kalanfa.utils.main._migrate_databases")
@patch("kalanfa.utils.main.version_updated")
