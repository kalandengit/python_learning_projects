"""
Tests for `kalanfa.utils.options` module.
"""
...
def test_option_reading_and_precedence_rules():
    ...
    with mock.patch.dict(
        os.environ, {"KALANFA_HOME": os.environ["KALANFA_HOME"]}, clear=True
    ):
    ...
    with mock.patch.dict(
        os.environ,
        {
            "KALANFA_LISTEN_PORT": str(_HTTP_PORT_ENV),
            "KALANFA_HOME": os.environ["KALANFA_HOME"],
        },
        clear=True,
    ):
        OPTIONS = options.read_options_file(ini_filename=tmp_ini_path)
    ...
    with mock.patch.dict(
        os.environ,
        {"KALANFA_HTTP_PORT": str(_HTTP_PORT_ENV), "KALANFA_LISTEN_PORT": "64000"},
    ):
        OPTIONS = options.read_options_file(ini_filename=tmp_ini_path)
        assert OPTIONS["Deployment"]["HTTP_PORT"] == _HTTP_PORT_ENV
...
def test_default_envvar_generation():
    ...
    with mock.patch.dict(
        os.environ,
        {"KALANFA_CONTENT_DIR": _CONTENT_DIR},
    ):
        OPTIONS = options.read_options_file(ini_filename=tmp_ini_path)
        assert OPTIONS["Paths"]["CONTENT_DIR"] == _CONTENT_DIR
...
def test_database_ssl_mode_envvar():
    ...
    with mock.patch.dict(
        ...
        {
            "KALANFA_HOME": os.environ["KALANFA_HOME"],
            "KALANFA_DATABASE_SSL_MODE": "require",
        },
        clear=True,
    ):
        OPTIONS = options.read_options_file(ini_filename=tmp_ini_path)
...
def test_improper_settings_display_errors_and_exit(monkeypatch):
    ...
    with activate_log_logger(monkeypatch):
        ...
        with mock.patch.dict(
            os.environ,
            {"KALANFA_HTTP_PORT": "baba", "KALANFA_HOME": os.environ["KALANFA_HOME"]},
            clear=True,
        ):
            with pytest.raises(SystemExit):
                options.read_options_file(ini_filename=tmp_ini_path)
...
def test_deprecated_envvars(monkeypatch):
    ...
    with activate_log_logger(monkeypatch):
        ...
        with mock.patch.dict(
            os.environ,
            {"KALANFA_LISTEN_PORT": "1234", "KALANFA_HOME": os.environ["KALANFA_HOME"]},
            clear=True,
        ):
            options.read_options_file(ini_filename=tmp_ini_path)
            assert any("deprecated" in msg[1] for msg in LOG_LOGGER)
...
def test_deprecated_aliases_envvars(monkeypatch):
    ...
    with activate_log_logger(monkeypatch):
        ...
        with open(tmp_ini_path, "w") as f:
            f.write("\n")
        with mock.patch.dict(os.environ, {"KALANFA_CACHE_REDIS_MIN_DB": "1234"}):
            options.read_options_file(ini_filename=tmp_ini_path)
            assert any("deprecated" in msg[1] for msg in LOG_LOGGER)
...
def test_path_expansion():
    """
    Checks that options under [Path] have "~" expanded, and are relativized to the KALANFA_HOME directory.
    """
    KALANFA_HOME_TEMP = tempfile.mkdtemp()
    logs_dir = os.path.join(KALANFA_HOME_TEMP, "logs")
    ...
    with mock.patch("kalanfa.utils.conf.KALANFA_HOME", KALANFA_HOME_TEMP):
        with mock.patch.dict(os.environ, {"KALANFA_CONTENT_DIR": absolute_path}):
            OPTIONS = options.read_options_file(ini_filename=tmp_ini_path)
            assert OPTIONS["Paths"]["CONTENT_DIR"] == absolute_path

        with mock.patch.dict(os.environ, {"KALANFA_CONTENT_DIR": "relative"}):
            OPTIONS = options.read_options_file(ini_filename=tmp_ini_path)
            assert OPTIONS["Paths"]["CONTENT_DIR"] == os.path.join(
                KALANFA_HOME_TEMP, "relative"
            )

        user_path = os.path.join("~", "homeiswherethecatis")

        with mock.patch.dict(os.environ, {"KALANFA_CONTENT_DIR": user_path}):
            OPTIONS = options.read_options_file(ini_filename=tmp_ini_path)
...
def test_lazy_function_import():
    ...
    dummy_function = options.LazyImportFunction(
        "kalanfa.utils.tests.do_not_import.dummy_function"
    )

    assert "kalanfa.utils.tests.do_not_import" not in sys.modules
    ...
    assert "kalanfa.utils.tests.do_not_import" in sys.modules
