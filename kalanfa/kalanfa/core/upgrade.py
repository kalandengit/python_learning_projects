...
import kalanfa
from kalanfa.utils.version import get_version_and_operator_from_range
from kalanfa.utils.version import normalize_version_to_semver
from kalanfa.utils.version import version_matches_range

CURRENT_VERSION = VersionInfo.parse(normalize_version_to_semver(kalanfa.__version__))
...
class VersionUpgrade:
    ...
    def __init__(self, old_version=None, new_version=None, upgrade=None):
        ...
        # of Kalanfa when this should be applied.
...
def get_upgrades(app_configs=None):
    if app_configs is None:
        app_configs = filter(
            lambda x: not getattr(x, "kalanfa_plugin", False), apps.get_app_configs()
        )
    version_upgrades = []
