...
from kalanfa.core.logger.constants import interaction_types
...
class ProgressTrackingViewSetStartSessionFreshTestCase(APITestCase):
    ...
    def _make_request(self, data, delete_keys=None):
        ...
        return self.client.post(
            reverse("kalanfa:core:trackprogress-list"),
            data=post_data,
            format="json",
        )
    ...
    def test_start_session_logged_in_lesson_succeeds(self):
        ...
        node_id = self.node.id
        with patch(
            "kalanfa.core.logger.viewsets.progress_tracking.wrap_to_save_queue"
        ) as save_queue_mock:
            response = self._make_request(
                {
                    "lesson_id": lesson_id,
...
class UpdateSessionBase:
    def _make_request(self, data):
        ...
        return self.client.put(
            reverse(
                "kalanfa:core:trackprogress-detail", kwargs={"pk": self.session_log.id}
            ),
            data=data,
            format="json",
        )
...
class ProgressTrackingViewSetAnonymousUpdateSessionTestCase(
    ...
    def test_wrong_user_session_404(self):
        ...
        response = self.client.put(
            reverse("kalanfa:core:trackprogress-detail", kwargs={"pk": session_log.id}),
            data={},
            format="json",
        )
...
class ProgressTrackingViewSetLoggedInUpdateSessionCoachQuizTestCase(
    ...
    def test_update_assessment_session_update_time_delta_succeeds(self):
        with patch("kalanfa.core.logger.viewsets.progress_tracking.wrap_to_save_queue"):
            response = self._make_request(
                {
                    "time_spent_delta": 5,
                }
    ...
    def test_update_session_absolute_progress_triggers_completion(self):
        with patch(
            ...
            response = self.client.put(
                reverse(
                    "kalanfa:core:trackprogress-detail",
...
class CSRFProtectedLoggerTestCase(APITestCase):
    ...
    def test_csrf_protected_trackprogress(self):
        ...
        url = reverse("kalanfa:core:trackprogress-list")
