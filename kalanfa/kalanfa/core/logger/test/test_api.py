...
from kalanfa.core.auth.management.commands.bulkexportusers import (
...
from kalanfa.core.logger.csv_export import labels
from kalanfa.core.logger.tasks import get_filepath
from kalanfa.core.logger.tasks import log_exports_cleanup
from kalanfa.core.utils.csv import open_csv_for_reading
        ...
        for content_index, content_id in enumerate(self.content_ids):
            ...
            for user_index, user in enumerate(self.users):
                ...
                response = self.client.get(
                    reverse("kalanfa:core:masterylog-list"),
                    data={
                        "content": content_id,
                        "user": user.id,
                        "complete": True,
    ...
    def test_diff(self):
        for user_index, user_tries in enumerate(self.user_tries):
            for try_index, user_try in enumerate(user_tries):
                ...
                response = self.client.get(
                    reverse("kalanfa:core:masterylog-diff", kwargs={"pk": try_index}),
                    data={
...
class TotalContentProgressViewSetTest(APITestCase):
    ...
    def test_summary_logs_only_no_complete(self):
        ...
        response = self.client.get(
            reverse("kalanfa:core:userprogress-detail", kwargs={"pk": self.user1.id}),
        )
