...
import kalanfa
from kalanfa.core.auth.constants.collection_kinds import FACILITY
from kalanfa.core.deviceadmin.management.commands.dbrestore import CommandError
from kalanfa.core.deviceadmin.utils import dbbackup
from kalanfa.core.deviceadmin.utils import dbrestore
from kalanfa.core.deviceadmin.utils import default_backup_folder
from kalanfa.core.deviceadmin.utils import get_dtm_from_backup_name
from kalanfa.core.deviceadmin.utils import IncompatibleDatabase
from kalanfa.core.deviceadmin.utils import search_latest
from kalanfa.utils.server import NotRunning
from kalanfa.utils.server import STATUS_UNKNOWN
...
def test_active_kalanfa():
    """
    Tests that we cannot restore while kalanfa is active
    """

    with patch(
        "kalanfa.utils.server.get_status",
...
def test_inactive_kalanfa():
    """
    Tests that we cannot restore while kalanfa is active
    """

    with patch(
        "kalanfa.utils.server.get_status", side_effect=mock_status_not_running
    ) as gs:
...
def test_restore_from_latest():
    ...
    if not is_sqlite_settings():
        return
    with patch("kalanfa.utils.server.get_status", side_effect=mock_status_not_running):
        ...
        sql = "syntax error;"
        fbroken = "db-v{}_2015-08-02_00-00-00.dump".format(kalanfa.__version__)
        open(os.path.join(default_backup_folder(), fbroken), "w").write(sql)

        # Add an unparsable file name
        fbroken = "db-v{}_.dump".format(kalanfa.__version__)
        open(os.path.join(default_backup_folder(), fbroken), "w").write(sql)
...
def test_search_latest():
    ...
    major_version = ".".join(kalanfa.__version__.split(".")[:2])

    files = [
        "db-v{}_2015-08-02_00-00-00.dump".format(kalanfa.__version__),
        "db-v{}_2016-08-02_00-00-00.dump".format(kalanfa.__version__),
        "db-v{}_2017-07-02_00-00-00.dump".format(major_version),
        "db-v{}_2017-08-02_00-00-00.dump".format(kalanfa.__version__),
    ]
