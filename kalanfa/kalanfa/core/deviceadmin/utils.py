...
def default_backup_folder():
    return os.path.join(KALANFA_HOME, "backups")


def get_backup_files():
    """
    Returns all backups from current kalanfa version.
    """
...
def is_full_version(fname):
    ...
    full_version = kalanfa.__version__
...
def dbbackup(old_version, dest_folder=None):
    ...
    to know which version of Kalanfa that a certain database should match.

    :param: dest_folder: Default is ~/.kalanfa/backups/db-[version]-[date].dump
    ...
    # See: https://github.com/learningequality/kalanfa/issues/2875
