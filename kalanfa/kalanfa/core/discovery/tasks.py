...
from kalanfa.core.device.task_notifications import status_fn
...
from kalanfa.core.discovery.models import DynamicNetworkLocation
from kalanfa.core.discovery.models import LocalHostname
from kalanfa.core.discovery.models import LocationTypes
from kalanfa.core.discovery.models import NetworkLocation
from kalanfa.core.discovery.models import StaticNetworkLocation
from kalanfa.core.discovery.utils.network.broadcast import KalanfaInstance
from kalanfa.core.discovery.utils.network.connections import update_network_location
...
from kalanfa.core.discovery.well_known import DATA_PORTAL_SYNCING_BASE_URL
...
def _store_dynamic_instance(broadcast_id, instance):
    ...
    :param instance: The new Kalanfa instance that has been discovered
    ...
    try:
        ...
        logger.warning(
            """
                A new Kalanfa instance '%s' was seen on the zeroconf network,
...
def hydrate_instance(func):
    """
    Small decorator that turns the `KalanfaInstance` dictionary/JSON into a `KalanfaInstance`
    """

    @functools.wraps(func)
    def wrapped(*args):
        new_args = list(args)
        new_args[1] = KalanfaInstance.from_dict(args[1])
...
def dispatch_broadcast_hooks(hook_type, instance):
    ...
    :type hook_type: str
    :param instance: The current Kalanfa instance that this device is broadcasting
    :type instance: kalanfa.core.discovery.utils.network.broadcast.KalanfaInstance
    """
...
def _refresh_reserved_locations():
    """
    Refreshes the reserved network locations for Studio and Kalanfa Data Portal
    """
    ...
    # Create or update Kalanfa Data Portal reserved location
    NetworkLocation.objects.update_or_create(
        id=DATA_PORTAL_BASE_INSTANCE_ID,
        defaults={
            "instance_id": DATA_PORTAL_BASE_INSTANCE_ID,
            "nickname": "Kalanfa Data Portal",
            "base_url": DATA_PORTAL_SYNCING_BASE_URL,
            "location_type": LocationTypes.Reserved,
            "is_local": False,
            "application": "Kalanfa Data Portal",
            "kalanfa_version": "0.16.0",
        },
    )
...
def sync_local_hostnames(hostnames):
    ...
    previously stored.
    :param hostnames: A list of str hostnames, e.g. ["kalanfa.local"]
    """
