...
from kalanfa.deployment.default.sqlite_db_names import NETWORK_LOCATION
...
class LocationTypes(ChoicesEnum):
    ...
    Static = "static"
    # dynamic locations discovered by the Kalanfa instance
    Dynamic = "dynamic"


class NetworkLocation(models.Model):
    """
    ``NetworkLocation`` stores information about a network address through which an instance of Kalanfa can be accessed,
    ...
    kalanfa_version = models.CharField(max_length=100, blank=True)
    ...
    @property
    def is_kalanfa(self):
        return self.application == "kalanfa"
    ...
    def matches_version(self, version):
        """
        Truncates the kalanfa version to the patch level (0.16.0a1 -> 0.16.0) and compares it with
        ...
        :return: a bool
        """
        return matches_version(truncate_version(self.kalanfa_version), version)
...
class LocalHostname(models.Model):
    """
    Stores a `.local` mDNS hostname (e.g. "kalanfa.local",
...
class NetworkLocationRouter(KalanfaModelRouter):
    """
