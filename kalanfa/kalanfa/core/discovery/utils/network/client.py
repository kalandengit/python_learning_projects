...
from kalanfa.core.utils.urls import join_url
...
class NetworkClient(SameHostSession):
    ...
    def connect(self, raise_if_unavailable=True):  # noqa: C901
        ...
        from kalanfa.core.device.utils import device_info_keys
        from kalanfa.core.device.utils import DEVICE_INFO_VERSION
        ...
        try:
            ...
            if self.device_info["application"] not in ["studio", "kalanfa"]:
                raise errors.NetworkLocationInvalidResponse(
                    "Server is not running Kalanfa or Studio"
                )
...
def get_user_agent():
    return "Kalanfa/{0} python-requests/{1}".format(
        kalanfa.__version__, requests.__version__
    )
