...
MOCK_PORT = 555
MOCK_ID = "abba"
LOCAL_HOSTNAMES_MODULE = "kalanfa.core.discovery.utils.network.local_hostnames."
...
class LocalHostnameListenerTestCase(TestCase):
    ...
    def setUp(self):
        super().setUp()
        self.instance = KalanfaInstance(MOCK_ID, ip=MOCK_INTERFACE_IP, port=MOCK_PORT)
        self.broadcast = KalanfaBroadcast(instance=self.instance)
    ...
    def test_update_local_names(self, mock_enqueue):
        hostnames = ["kalanfa.local", "tonyslaptop.local"]
