...
class NetworkClientTestCase(TestCase):
    ...
    def test_discover_from_address__not_found(self):
        with self.assertRaises(errors.NetworkLocationNotFound):
            NetworkClient.discover_from_address("nonkalanfahappyurl.qqq")
    ...
    def test_request__user_agent(self):
        ...
        self.assertIn(
            "Kalanfa/{0}".format(kalanfa.__version__), client.headers["User-Agent"]
        )
    ...
    def test_connect__not_a_kalanfa(self):
        response = mock_response(200)
        response.json.return_value = {"application": "not-a-kalanfa"}
