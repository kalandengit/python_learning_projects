...
def patch_os_path_exists_for_kalanfa_folder(folder_lookup):
    def wrapper(f):
        def check_os_path_exists(path):
            if not path.endswith(EXPORT_FOLDER_NAME):
                raise Exception(
                    "Checking os.path.exists only mocked for kalanfa data folder paths."
                )
...
class WindowsFilesystemTestCase(TestCase):
    ...
    @patch_os_path_exists_for_kalanfa_folder(windows_data.has_kalanfa_data_folder)
    @patch(
        "kalanfa.core.discovery.utils.filesystem.windows._wmic_output",
        mocked_wmic_output,
    )
    def setUp(self):
    ...
    @patch("kalanfa.core.discovery.utils.filesystem.windows._wmic_output")
...
class LinuxFilesystemTestCase(TestCase):
    ...
    @patch_os_path_exists_for_kalanfa_folder(linux_data.has_kalanfa_data_folder)
...
class OSXFilesystemTestCase(TestCase):
    ...
    @patch_os_path_exists_for_kalanfa_folder(osx_data.has_kalanfa_data_folder)
