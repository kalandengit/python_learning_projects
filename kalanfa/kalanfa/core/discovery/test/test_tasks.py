...
MOCK_PORT = 555
SEARCH_MODULE = "kalanfa.core.discovery.utils.network.search."
...
class PerformNetworkLocationUpdateTestCase(TestCase):
    ...
    def setUp(self):
        self.network_location = DynamicNetworkLocation.objects.create(
            ...
            connection_faults=0,
            application="kalanfa",
            kalanfa_version="0.15.11",
            instance_id="b" * 32,
            subset_of_users_device=False,
        )

    @mock.patch("kalanfa.core.discovery.tasks._update_connection_status")
    def test_not_found(self, mock_update):
        perform_network_location_update("c" * 32)
        mock_update.assert_not_called()

    @mock.patch(
        "kalanfa.core.discovery.tasks._enqueue_network_location_update_with_backoff"
    )
    @mock.patch("kalanfa.core.discovery.tasks._update_connection_status")
...
class AddDynamicNetworkLocationTestCase(TestCase):
    ...
    @mock.patch("kalanfa.core.discovery.tasks.perform_network_location_update.enqueue")
    @mock.patch("kalanfa.core.discovery.tasks._store_dynamic_instance")
    ...
    @mock.patch("kalanfa.core.discovery.tasks.get_device_setting", return_value=True)
    @mock.patch("kalanfa.core.discovery.tasks.perform_network_location_update.enqueue")
...
class RemoveDynamicNetworkLocationTestCase(TestCase):
    ...
    @mock.patch("kalanfa.core.discovery.tasks._dispatch_discovery_hooks")
...
class DispatchBroadcastHooksTestCase(TestCase):
    ...
    def setUp(self):
        ...
        broadcast_hooks = mock.patch(
            "kalanfa.core.discovery.tasks.NetworkLocationBroadcastHook"
        )
    ...
    def test_okay_dispatch(self):
        ...
        self.assertIsInstance(call[0], KalanfaInstance)
...
class SyncLocalHostnamesTestCase(TestCase):
    ...
    def test_creates_new_hostnames(self):
        self.task(["kalanfa.local", "tonyslaptop.local"])
        self.assertEqual(
            {"kalanfa.local", "tonyslaptop.local"},
            set(LocalHostname.objects.values_list("hostname", flat=True)),
        )

    def test_removes_stale_hostnames(self):
        LocalHostname.objects.create(hostname="oldname.kalanfa.local")
        self.task(["kalanfa.local"])
        self.assertEqual(
            ["kalanfa.local"],
...
class TaskUtilitiesTestCase(TestCase):
    ...
    @mock.patch("kalanfa.core.discovery.tasks.NetworkLocationDiscoveryHook")
    ...
    @mock.patch("kalanfa.core.discovery.tasks._dispatch_discovery_hooks")
    @mock.patch("kalanfa.core.discovery.tasks.update_network_location")
    ...
    def test_hydrate_instance(self):
        ...
        def test_func(other_arg, instance):
            self.assertIsInstance(instance, KalanfaInstance)
    ...
    @mock.patch("kalanfa.core.discovery.tasks.get_current_job")
    ...
    def test_enqueue_network_location_update_with_backoff__not_local(
        ...
        with mock.patch("kalanfa.core.discovery.tasks.logger") as mock_logger:
            _enqueue_network_location_update_with_backoff(self.network_location)
