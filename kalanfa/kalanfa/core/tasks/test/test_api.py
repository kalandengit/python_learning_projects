...
from kalanfa.core.tasks.exceptions import JobNotFound
...
from kalanfa.core.tasks.permissions import CanManageContent
from kalanfa.core.tasks.permissions import IsSuperAdmin
from kalanfa.core.tasks.registry import RegisteredTask
from kalanfa.core.tasks.registry import TaskRegistry
...
@patch("kalanfa.core.tasks.viewsets.tasks.job_storage")
class TaskAPITestCase(BaseAPITestCase):
    ...
    def test_task_cancel(self, job_storage_mock):
        ...
        response = self.client.post(
            reverse("kalanfa:core:task-cancel", kwargs={"pk": "1"}), format="json"
        )
...
class CreateTaskAPITestCase(BaseAPITestCase):
    ...
    def test_api_validator_task_field_check(self, mock_job_storage):
        # When "task" is absent.
        response = self.client.post(
            reverse("kalanfa:core:task-list"), {"x": 0, "y": 42}, format="json"
        )
        self.assertEqual(response.status_code, 400)

        response = self.client.post(
            reverse("kalanfa:core:task-list"),
            [{"x": 0, "y": 42}, {"x": 0, "y": 42}],
            format="json",
        )
        self.assertEqual(response.status_code, 400)
    ...
    def test_api_validator_unregistered_task(self, mock_job_storage):
        ...
        response = self.client.post(
            reverse("kalanfa:core:task-list"),
            {
                "type": "kalanfa.core.tasks.test.test_api.add",
                "x": 0,
                "y": 42,
            },
            format="json",
    ...
    def test_api_validator_handles_task_permissions(self, mock_job_storage):
        ...
        TaskRegistry["kalanfa.core.tasks.test.test_api.add"] = add
        ...
        response = self.client.post(
            reverse("kalanfa:core:task-list"),
            {"type": "kalanfa.core.tasks.test.test_api.add", "x": 0, "y": 42},
            format="json",
        )
    ...
    def test_api_handles_single_task_without_validator(self, mock_job_storage):
        ...
        response = self.client.post(
            reverse("kalanfa:core:task-list"),
            {"type": "kalanfa.core.tasks.test.test_api.add", "kalanfa": "fly"},
            format="json",
        )
    ...
    def test_api_handles_bulk_task_without_validator(self, mock_job_storage):
        ...
        for call_args in mock_job_storage.call_args_list:
            self.assertEqual(call_args[0][0].kwargs, {"kalanfa": "fly"})
            self.assertEqual(
    ...
    def test_api_handles_single_task_with_validator(self, mock_job_storage):
        ...
        class add_validator(JobValidator):
            ...
            def validate(self, data):
                ...
                return {
                    "kwargs": {"x": 0, "y": 42},
                    "extra_metadata": {"facility": "kalanfa HQ"},
                }
...
class EnqueueArgsCreateAPITestCase(BaseAPITestCase):
    ...
    def setUp(self):
        ...
        TaskRegistry["kalanfa.core.tasks.test.test_api.life"] = life
        self.registered_task = TaskRegistry["kalanfa.core.tasks.test.test_api.life"]
        self.client.login(username=self.superuser.username, password=DUMMY_PASSWORD)
    ...
    def test_erroneous_request(self, mock_job_storage):
        ...
        for err_enq_arg in erroneous_enqueue_args:
            response = self.client.post(
                reverse("kalanfa:core:task-list"),
                {
                    "type": "kalanfa.core.tasks.test.test_api.life",
...
class EnqueueArgsUpdateAPITestCase(BaseAPITestCase):
    ...
    def setUp(self):
        ...
        self.fake_job = fake_job(
            state=State.QUEUED,
            job_id="test-id",
            func="kalanfa.core.tasks.test.test_api.life",
        )
        self.client.login(username=self.superuser.username, password=DUMMY_PASSWORD)
    ...
    def test_erroneous_request(self, mock_job_storage):
        ...
        for err_enq_arg in erroneous_enqueue_args:
            response = self.client.patch(
                reverse("kalanfa:core:task-detail", kwargs={"pk": "test-id"}),
                {
                    "enqueue_args": err_enq_arg,
                },
                format="json",
...
class ListAPIRepeat(BaseAPITestCase):
    ...
    def test_list_api_repeating_true(self, mock_job_storage):
        ...
        response = self.client.get(
            reverse("kalanfa:core:task-list"), {"repeating": "true"}
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_list_api_repeating_false(self, mock_job_storage):
        mock_job_storage.get_all_jobs.return_value = []
        response = self.client.get(
            reverse("kalanfa:core:task-list"), {"repeating": "false"}
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_list_api_repeating_invalid_value(self, mock_job_storage):
        mock_job_storage.get_all_jobs.return_value = []
        response = self.client.get(
            reverse("kalanfa:core:task-list"), {"repeating": "typo"}
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
    ...
    def test_list_api_repeating_not_present(self, mock_job_storage):
        mock_job_storage.get_all_jobs.return_value = []
        response = self.client.get(reverse("kalanfa:core:task-list"))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            mock_job_storage.get_all_jobs.call_args[1],
            {"queue": None, "repeating": None},
...
class TaskManagementAPITestCase(BaseAPITestCase):
    def setUp(self):
        @register_task(permission_classes=[IsSuperAdmin], queue="kalanfa")
        def add(x, y):
            return x + y

        @register_task(permission_classes=[CanManageContent], queue="kalanfa")
        def multiply(x, y):
            return x * y
        ...
        TaskRegistry["kalanfa.core.tasks.test.test_api.add"] = add
        TaskRegistry["kalanfa.core.tasks.test.test_api.multiply"] = multiply
        TaskRegistry["kalanfa.core.tasks.test.test_api.subtract"] = subtract
        ...
        self.jobs_response = [
            ...
            },
            {
                ...
                "traceback": "",
                "percentage": 0,
                "type": "kalanfa.core.tasks.test.test_api.multiply",
                "id": "1",
                "cancellable": False,
                "clearable": False,
                "facility_id": self.superuser.facility_id,
            ...
            },
            {
                ...
                "traceback": "",
                "percentage": 0,
                "type": "kalanfa.core.tasks.test.test_api.subtract",
                "id": "2",
                "cancellable": False,
                "clearable": False,
                "facility_id": self.facility2user.facility_id,
    ...
    def test_can_list_queue_specific_jobs(self, mock_job_storage):
        ...
        response = self.client.get(
            reverse("kalanfa:core:task-list"), {"queue": "kalanfa"}
        )

        self.assertEqual(response.data, self.jobs_response[:2])
        mock_job_storage.get_all_jobs.assert_called_once_with(
            queue="kalanfa", repeating=None
        )
    ...
    def test_can_superuser_retrieve_any_job(self, mock_job_storage):
        ...
        response = self.client.get(
            reverse("kalanfa:core:task-detail", kwargs={"pk": "0"})
        )
        ...
        mock_job_storage.get_orm_job.return_value = dummy_orm_job_data
        response = self.client.get(
            reverse("kalanfa:core:task-detail", kwargs={"pk": "2"})
        )
        self.assertEqual(response.data, self.jobs_response[2])
    ...
    def test_can_manage_content_can_retrieve_can_manage_content_jobs(
        ...
        response = self.client.get(
            reverse("kalanfa:core:task-detail", kwargs={"pk": "1"})
        )
    ...
    def test_retrieval_404(self, mock_job_storage):
        ...
        response = self.client.get(
            reverse("kalanfa:core:task-detail", kwargs={"pk": "3"})
        )
        self.assertEqual(response.status_code, 404)
    ...
    def test_restart_task(self, mock_job_storage):
        ...
        response = self.client.post(
            reverse("kalanfa:core:task-restart", kwargs={"pk": "2"}), format="json"
        )
    ...
    def test_restart_task_respect_permissions(self, mock_job_storage):
        ...
        response = self.client.post(
            reverse("kalanfa:core:task-restart", kwargs={"pk": "0"}), format="json"
        )
    ...
    def test_cancel_task_respect_permissions(self, mock_job_storage):
        ...
        response = self.client.post(
            reverse("kalanfa:core:task-cancel", kwargs={"pk": "0"}), format="json"
        )
    ...
    def test_clear_task_respect_permissions(self, mock_job_storage):
        ...
        response = self.client.post(
            reverse("kalanfa:core:task-clear", kwargs={"pk": "0"}), format="json"
        )
...
class TaskAPIPermissionsTestCase(APITestCase):
    ...
    def test_list_permissions(self, job_storage_mock):
        response = self.client.get(reverse("kalanfa:core:task-list"), format="json")
        self.assertEqual(response.status_code, 200)
...
class CSRFProtectedTaskTestCase(APITestCase):
    ...
    def test_csrf_protected_task(self):
        response = self.client_csrf.post(
            reverse("kalanfa:core:task-list"), {}, format="json"
        )
