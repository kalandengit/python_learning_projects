...
from kalanfa.core.auth.constants.morango_sync import State as FacilitySyncState
...
from kalanfa.core.auth.tasks import cleanupsync
from kalanfa.core.auth.tasks import CleanUpSyncsValidator
from kalanfa.core.auth.tasks import enqueue_soud_sync_processing
from kalanfa.core.auth.tasks import PeerFacilityImportJobValidator
from kalanfa.core.auth.tasks import PeerFacilitySyncJobValidator
from kalanfa.core.auth.tasks import soud_sync_processing
from kalanfa.core.auth.tasks import SyncJobValidator
...
class FacilityTasksAPITestCase(APITestCase):
    ...
    def test_startdataportalsync(self, mock_job_storage):
        ...
        response = self.client.post(
            reverse("kalanfa:core:task-list"),
            {
                ...
                "type": "kalanfa.core.auth.tasks.dataportalsync",
            },
            format="json",
        )
        self.assertEqual(response.status_code, 200)
    ...
    @patch("kalanfa.core.auth.tasks.NetworkClient")
    @patch("kalanfa.core.auth.tasks.validate_and_create_sync_credentials")
    def test_startpeerfacilityimport(
        ...
        req_data = dict(
            facility=self.facility.id,
            type="kalanfa.core.auth.tasks.peerfacilityimport",
            username="testuser",
            password="testpass",
            device_id=device.id,
        )
    ...
    def test_startpeerfacilitysync(
        ...
        req_data = dict(
            facility=self.facility.id,
            type="kalanfa.core.auth.tasks.peerfacilitysync",
            username="testuser",
            password="testpass",
            device_id=device.id,
        )
    ...
    def test_startdeletefacility(self, mock_job_storage):
        ...
        response = self.client.post(
            reverse("kalanfa:core:task-list"),
            dict(facility=facility2.id, type="kalanfa.core.auth.tasks.deletefacility"),
            format="json",
        )
        self.assertEqual(response.status_code, 200)
        self.assertJobResponse(fake_job_data, response)
    ...
    def test_startdeletefacility__not_superuser(self, mock_job_storage):
        ...
        response = self.client.post(
            reverse("kalanfa:core:task-list"),
            dict(facility=facility1.id, type="kalanfa.core.auth.tasks.deletefacility"),
            format="json",
        )
    ...
    def test_startdeletefacility__facility_member(self, mock_job_storage):
        ...
        response = self.client.post(
            reverse("kalanfa:core:task-list"),
            dict(
                facility=self.facility.id, type="kalanfa.core.auth.tasks.deletefacility"
            ),
            format="json",
        )
...
class FacilityTaskHelperTestCase(TestCase):
    ...
    def test_validate_no_facility__validation_error(self):
        with self.assertRaises(serializers.ValidationError):
            SyncJobValidator(
                data={"type": "kalanfa.core.auth.tasks.peerfacilitysync"}
            ).is_valid(raise_exception=True)

    def test_validate_empty_facility_id__validation_error__empty(self):
        with self.assertRaises(serializers.ValidationError):
            SyncJobValidator(
                data=dict(type="kalanfa.core.auth.tasks.peerfacilitysync", facility="")
            ).is_valid(raise_exception=True)
    ...
    def test_validated_data(self):
        ...
        validator = SyncJobValidator(
            data=dict(
                type="kalanfa.core.auth.tasks.peerfacilitysync", facility=facility_id
            )
        )
    ...
    @patch("kalanfa.core.auth.utils.sync.MorangoProfileController")
    @patch("kalanfa.core.auth.tasks.NetworkClient")
    @patch("kalanfa.core.auth.utils.sync.get_client_and_server_certs")
    @patch("kalanfa.core.auth.utils.sync.get_facility_dataset_id")
...
class SoudTasksTestCase(TestCase):
    @patch("kalanfa.core.auth.tasks.job_storage")
    @patch("kalanfa.core.auth.tasks.soud_sync_processing")
    @patch("kalanfa.core.auth.tasks.soud")
    ...
    @patch("kalanfa.core.auth.tasks.get_current_job")
    @patch("kalanfa.core.auth.tasks.soud")
...
class CleanUpSyncsTaskTestCase(TestCase):
    ...
    @patch("kalanfa.core.auth.tasks.CleanUpSyncsValidator")
    ...
    @patch("kalanfa.core.auth.tasks.call_command")
    ...
    def test_actual_run__not_provisioned(self):
        clear_process_cache()
        with self.assertRaisesRegex(CommandError, "Kalanfa is unprovisioned"):
            cleanupsync(**self.kwargs)
