...
from kalanfa.core.auth.kalanfa_plugin import PicturePasswordsSyncHook
...
class PicturePasswordsSyncHookTestCase(TestCase):
    ...
    @mock.patch(
        "kalanfa.core.auth.kalanfa_plugin.are_picture_passwords_exhausted",
        return_value=True,
    )
    ...
    @mock.patch("kalanfa.core.auth.kalanfa_plugin.assign_picture_password")
    def test_catches_no_available_sequences_and_stops(self, mock_assign):
        ...
        with self.assertLogs("kalanfa.core.auth.kalanfa_plugin", level="ERROR"):
            self._call_post_transfer()
