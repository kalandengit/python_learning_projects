...
from kalanfa.core.auth.constants.picture_passwords import PICTURE_PASSWORD_SET
...
from kalanfa.core.auth.utils.picture_passwords import get_all_valid_sequences
from kalanfa.core.auth.utils.picture_passwords import get_assigned_sequences
from kalanfa.core.auth.utils.sync import ClassroomPartitionFilterFactory
from kalanfa.core.upgrade import matches_version
from kalanfa.utils.version import truncate_version

from .sync_event_hook_utils import get_dataset_id
from .sync_event_hook_utils import get_other_side_kalanfa_version
...
class KalanfaLocalInitializeOperation(InitializeOperation):
    """
    ...
    functionality to Kalanfa, and Kalanfa already has plugins which modify the sync behavior, it's
...
class KalanfaNetworkInitializeOperation(NetworkInitializeOperation):
    """
...
class KalanfaSyncOperations(BaseOperation):
    """
    Proxy class for Morango sync operations which allows customized behavior through Kalanfa plugins
    """

    def handle(self, context):
        """
        Kalanfa plugins can register transfer operations to alter the behavior of the sync. The
...
class KalanfaSyncOperationMixin(BaseOperation):
    """
...
class KalanfaVersionedSyncOperation(KalanfaSyncOperationMixin, LocalOperation):
    """
    ...
    def handle_initial(self, context):
        ...
        # get the kalanfa version, which is defined in
        # kalanfa.core.auth.constants.morango_sync:CUSTOM_INSTANCE_INFO
        remote_version = get_other_side_kalanfa_version(context)

        # pre-0.15.0 won't have the kalanfa version
...
class KalanfaSingleUserSyncOperation(KalanfaSyncOperationMixin, LocalOperation):
    """
