...
from kalanfa.core.auth.constants.demographics import choices as GENDER_CHOICES
from kalanfa.core.auth.constants.demographics import custom_demographics_schema
from kalanfa.core.auth.constants.demographics import DEFERRED
from kalanfa.core.auth.constants.demographics import DescriptionTranslationValidator
from kalanfa.core.auth.constants.demographics import EnumValuesValidator
from kalanfa.core.auth.constants.demographics import FacilityUserDemographicValidator
from kalanfa.core.auth.constants.demographics import LabelTranslationValidator
from kalanfa.core.auth.constants.demographics import NOT_SPECIFIED
from kalanfa.core.auth.constants.demographics import UniqueIdsValidator
from kalanfa.core.auth.constants.morango_sync import ScopeDefinitions
...
from kalanfa.core.device.utils import is_full_facility_import
...
from kalanfa.core.utils.model_router import KalanfaModelRouter
from kalanfa.core.utils.validators import JSON_Schema_Validator
from kalanfa.deployment.default.sqlite_db_names import SESSIONS
...
class SessionRouter(KalanfaModelRouter):
    """
...
class FacilityDataset(FacilityDataSyncableModel):
    ...
    def reset_to_default_settings(self, preset=None):
        from kalanfa.core.auth.constants.facility_presets import mappings
...
class AbstractFacilityDataModel(FacilityDataSyncableModel):
    """
    Base model for Kalanfa "Facility Data", which is data that is specific to a particular ``Facility``,
    ...
    def pre_save(self, **kwargs):
        ...
        try:
            self.ensure_dataset()
        except KalanfaValidationError as e:
    ...
    def ensure_dataset(self, *args, **kwargs):
        ...
        inconsistencies, make sure it matches the inferred dataset, otherwise raise a ``KalanfaValidationError``.
        If we have no dataset and it can't be inferred, we raise a ``KalanfaValidationError`` exception as well.
        """
...
def validate_username(value):
    try:
        ...
        # for kalanfa backwards compatibility, if the username is not an email:
        validate_username_allowed_chars(value)
        validate_username_max_length(value)


class KalanfaBaseUserMixin:
    """
    This mixin is inherited by ``KalanfaAnonymousUser`` and ``FacilityUser``.
    ...
    def session_data(self):
        ...
        """
        raise NotImplementedError(
            "Subclasses of KalanfaBaseUserMixin must override the `session_data` property."
        )
    ...
    def is_member_of(self, coll):
        ...
        """
        raise NotImplementedError(
            "Subclasses of KalanfaBaseUserMixin must override the `is_member_of` method."
        )
    ...
    def has_role_for_user(self, kinds, user):
        ...
        :type kinds: string from ``kalanfa.core.auth.constants.role_kinds.*``
        :return: ``True`` if this user has the specified role kind with respect to the target user, otherwise ``False``.
        :rtype: bool
        """
        raise NotImplementedError(
            "Subclasses of KalanfaBaseUserMixin must override the `has_role_for_user` method."
        )
    ...
    def has_role_for_collection(self, kinds, coll):
        ...
        """
        raise NotImplementedError(
            "Subclasses of KalanfaBaseUserMixin must override the `has_role_for_collection` method."
        )
    ...
    def can_create_instance(self, obj):
        ...
        This method should be overridden by classes that inherit from ``KalanfaBaseUserMixin``.
        ...
        """
        raise NotImplementedError(
            "Subclasses of KalanfaBaseUserMixin must override the `can_create_instance` method."
        )
    ...
    def can_read(self, obj):
        ...
        """
        raise NotImplementedError(
            "Subclasses of KalanfaBaseUserMixin must override the `can_read` method."
        )
    ...
    def can_update(self, obj):
        ...
        """
        raise NotImplementedError(
            "Subclasses of KalanfaBaseUserMixin must override the `can_update` method."
        )
    ...
    def can_delete(self, obj):
        ...
        """
        raise NotImplementedError(
            "Subclasses of KalanfaBaseUserMixin must override the `can_delete` method."
        )
    ...
    def has_role_for(self, kinds, obj):
        ...
        """
        if isinstance(obj, KalanfaBaseUserMixin):
            return self.has_role_for_user(kinds, obj)
        elif isinstance(obj, Collection):
            return self.has_role_for_collection(kinds, obj)
        else:
            raise ValueError(
                "The `obj` argument to `has_role_for` must be either an instance of KalanfaBaseUserMixin or Collection."
            )
...
class KalanfaAnonymousUser(AnonymousUser, KalanfaBaseUserMixin):
    """
    Custom anonymous user that also exposes the same interface as KalanfaBaseUserMixin, for consistency.
    """
...
class FacilityUser(AbstractBaseUser, KalanfaBaseUserMixin, AbstractFacilityDataModel):
    """
...
class Facility(Collection):
    ...
    def infer_dataset(self, *args, **kwargs):
        ...
        if not self.dataset_id:
            from kalanfa.core.device.models import DEFAULT_DEMOGRAPHIC_FIELDS_KEY
