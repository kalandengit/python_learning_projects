...
from kalanfa.core.errors import RedisConnectionError
from kalanfa.core.sqlite.pragmas import CONNECTION_PRAGMAS
from kalanfa.core.sqlite.pragmas import START_PRAGMAS
from kalanfa.core.sqlite.utils import repair_sqlite_db
from kalanfa.core.utils.cache import process_cache
from kalanfa.core.utils.cache import RedisSettingsHelper
...
class KalanfaCoreConfig(AppConfig):
    name = "kalanfa.core"
    ...
    def ready(self):
        ...
        # Log the settings file that we are running Kalanfa with.
        ...
        logger.info(
            "Running Kalanfa with the following settings: {settings}".format(
        ...
        # Register any django apps that may have kalanfa plugin
        # modules inside them
    ...
    def check_file_storage_settings():
        ...
        if OPTIONS["FileStorage"]["STORAGE_BACKEND"] == "gcs":
            ...
            default_storage.get_available_name("kalanfa")
        ...
        try:
            ...
            if (
                ...
                logger.warning(
                    ...
                    "Kalanfa, the following is suggested: maxmemory-policy allkeys-lru"
                )
