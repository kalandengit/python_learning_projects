...
from kalanfa.core.upgrade import VersionUpgrade
...
def test_filter_old_version():
    ...
    with patch(
        "kalanfa.core.upgrade.get_upgrades", return_value=[not_filtered, filtered]
    ):
...
def test_not_filter_alpha_version():
    ...
    with patch("kalanfa.core.upgrade.get_upgrades", return_value=[not_filtered]):
...
def test_order_old_version():
    ...
    with patch("kalanfa.core.upgrade.get_upgrades", return_value=[second, first]):
...
def test_blank_old_version():
    ...
    with patch("kalanfa.core.upgrade.get_upgrades", return_value=[first]):
        run_upgrades("", "1.1.2")
