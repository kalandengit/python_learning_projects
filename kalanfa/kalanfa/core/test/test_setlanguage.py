...
class I18NTests(TestCase):
    databases = "__all__"
    """
    Tests set_language view in kalanfa/core/views.py
    ...
    def test_setlang(self):
        ...
        response = self.client.post(reverse("kalanfa:core:set_language"), post_data)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.content.decode("utf-8"),
            translate_url(reverse("kalanfa:core:redirect_user"), lang_code),
        )
    ...
    def test_setlang_next_valid(self):
        ...
        """
        lang_code = self._get_inactive_language_code()
        next_url = reverse("kalanfa:kalanfa.plugins.learn:learn")
        post_data = dict(language=lang_code, next=next_url)
        response = self.client.post(reverse("kalanfa:core:set_language"), post_data)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.content.decode("utf-8"),
            translate_url(reverse("kalanfa:kalanfa.plugins.learn:learn"), lang_code),
        )
        self.assertEqual(self.client.session[LANGUAGE_SESSION_KEY], lang_code)
    ...
    def test_setlang_null(self):
        ...
        self.assertEqual(
            response.content.decode("utf-8"),
            translate_url(reverse("kalanfa:core:redirect_user"), current_language),
        )
    ...
    def test_setlang_null_next_valid(self):
        ...
        self.assertEqual(
            response.content.decode("utf-8"),
            translate_url(
                reverse("kalanfa:kalanfa.plugins.learn:learn"), current_language
            ),
        )
        self.assertNotIn(LANGUAGE_SESSION_KEY, self.client.session)
    ...
    def test_setlang_get(self):
        ...
        lang_code = self._get_inactive_language_code()
        post_data = dict(language=lang_code)
        response = self.client.get(reverse("kalanfa:core:set_language"), data=post_data)
