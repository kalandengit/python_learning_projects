...
class LocalNotificationAPITestCase(APITestCase):
    ...
    def test_list_returns_rows_for_superuser(self):
        ...
        response = self.client.get(reverse("kalanfa:core:localnotification-list"))
    ...
    def test_destroy_deletes_row(self):
        ...
        self._login_superuser()
        detail_url = reverse(
            "kalanfa:core:localnotification-detail", kwargs={"pk": notif.pk}
        )
...
class ImpactStoriesTriggerTests(TestCase):
    ...
    @mock.patch("kalanfa.core.analytics.local_notifications.ACTIVITY_THRESHOLD", 5)
    @mock.patch("kalanfa.core.analytics.local_notifications.ROLLING_WINDOW_DAYS", 90)
    ...
    @mock.patch("kalanfa.core.analytics.local_notifications.ACTIVITY_THRESHOLD", 100)
    ...
    @mock.patch("kalanfa.core.analytics.local_notifications.ACTIVITY_THRESHOLD", 5)
    @mock.patch("kalanfa.core.analytics.local_notifications.ROLLING_WINDOW_DAYS", 30)
...
class LocalNotificationTaskTests(TestCase):
    @mock.patch("kalanfa.core.analytics.tasks._generate_local_notifications.enqueue_in")
    @mock.patch(
        "kalanfa.core.analytics.tasks.create_impact_stories_notification_if_needed",
        return_value=True,
    )
