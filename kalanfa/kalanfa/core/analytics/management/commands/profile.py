...
from kalanfa.utils.server import PROFILE_LOCK
...
def remove_lock():
    # Kalanfa command PID file exists but no command is running, it's corrupted
    try:
...
class Command(BaseCommand):
    ...
    - Timestamp
    - Number of Kalanfa active sessions
    - Number of Kalanfa active users
    - Number of Kalanfa active users that have interacted in the last minute
    ...
    - Percentage of use of cpu by the Kalanfa process
    - Memory (In Mbytes) used by the kalanfa process (just RAM, not swap memory included)
    """

    help = "Logs performance/profiling info in the server running Kalanfa"
    ...
    def check_start_conditions(self):
        ...
        if not conf.OPTIONS["Server"]["PROFILE"]:
            logger.error(
                "Kalanfa has not enabled profiling of its requests."
                "To enable it, edit the Kalanfa options.ini file and "
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            logger.error(
                "Impossible to create profile lock file. Kalanfa won't profile its requests"
            )
        samples = 1
        num_samples = options["num_samples"]
        performance_dir = os.path.join(conf.KALANFA_HOME, "performance")
