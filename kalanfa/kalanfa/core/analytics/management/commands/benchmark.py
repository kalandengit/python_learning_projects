...
from kalanfa.core.analytics.measurements import get_channels_usage_info
from kalanfa.core.analytics.measurements import get_db_info
from kalanfa.core.analytics.measurements import get_kalanfa_process_cmd
from kalanfa.core.analytics.measurements import get_kalanfa_use
from kalanfa.core.analytics.measurements import get_machine_info
from kalanfa.core.analytics.measurements import get_requests_info
...
class Command(BaseCommand):
    """
    This command will output information about different parameters of the server running Kalanfa
    Output example:
    ...
    * Total processes:               351
    * Used CPU:                      33.6%
    * Kalanfa CPU usage:             22.3%

    Memory
    * Used memory:                   9.3 GB
    * Total memory:                  16.0 GB
    * Kalanfa memory usage:          56.8 MB
    ...
    * Server timezone:               (server_timezone)
    ...
    help = "Outputs performance info and statistics of usage for the running Kalanfa instance in this server"
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            sys.exit("Profile command executed while Kalanfa server was not running")
        get_requests_info()
        self.messages = []
        ...
        self.add_header("CPU")
        kalanfa_cpu, kalanfa_mem = get_kalanfa_use()
        used_cpu, used_memory, total_memory, total_processes = get_machine_info()
        cpu_parameters = ("Total processes", "Used CPU", "Kalanfa CPU usage")
        cpu_values = (
            total_processes,
            "{} %".format(used_cpu),
            "{} %".format(kalanfa_cpu),
        ...
        memory_parameters = ("Used memory", "Total memory", "Kalanfa memory usage")
        memory_values = (
            "{} Mb".format(used_memory),
            "{} Mb".format(total_memory),
            "{} Mb".format(kalanfa_mem),
        )
        ...
        self.messages.append(format_line("Version", kalanfa.__version__))
        self.messages.append(format_line("OS", instance_model.platform))
        self.messages.append(
            format_line("Installer", installation_type(get_kalanfa_process_cmd()))
        )
        self.messages.append(
