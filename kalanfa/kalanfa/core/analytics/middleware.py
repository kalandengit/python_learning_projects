...
try:
    ...
    kalanfa_process = psutil.Process()
except NotImplementedError:
    # This middleware can't work on this OS
    kalanfa_process = None
...
def cherrypy_access_log_middleware(get_response):
    ...
    def middleware(request):
        ...
        # https://github.com/learningequality/kalanfa/issues/6459
        log_as_debug = (
...
class Metrics:
    ...
    def get_used_memory(self):
        return kalanfa_process.memory_info().vms

    def get_load_average(self):
        return kalanfa_process.cpu_percent()

    def get_stats(self):
        """
        Calcutes time spent in processing the request
        and difference in memory and load consumed
        by kalanfa while processing the request
        :returns: tuple of strings containing time consumed (in seconds),
                  Kalanfa used memory (in bytes) before and after executing the request,
                  Kalanfa cpu load (in %) before and after executing the request.
        """
...
class MetricsMiddleware(MiddlewareMixin):
    ...
    - Memory (in Kbytes) used by the kalanfa process when the request came in
    - Memory (in Kbytes) used by the kalanfa process when the response was sent
    - Percentage of use of cpu by the Kalanfa process when the request came in
    - Percentage of use of cpu by the Kalanfa process when the response was sent
        ...
        if MetricsMiddleware.disabled and conf.OPTIONS["Server"]["PROFILE"]:
            if os.path.exists(PROFILE_LOCK):
                try:
                    ...
                    # Kalanfa command PID file has been deleted or it's corrupted
                    try:
                        os.remove(PROFILE_LOCK)
                    except OSError:
                        pass  # lock file was deleted by other process
