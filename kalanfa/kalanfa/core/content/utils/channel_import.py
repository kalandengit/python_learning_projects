...
from kalanfa.core.content.constants.schema_versions import VERSION_5
from kalanfa.core.content.legacy_models import License
...
from kalanfa.core.content.utils.annotation import update_channel_version_to_assignments
...
from kalanfa.core.errors import KalanfaUpgradeError
...
CONTENT_APP_NAME = KalanfaContentConfig.label
...
class ChannelImport:
    ...
    with the current version of Kalanfa (i.e. no explicit mappings are required to bring data from the content db
    ...
    # Specific instructions and exceptions for importing table from previous versions of Kalanfa
    # Mappings can be:
    ...
    def postgres_table_import(self, model, row_mapper, table_mapper):
        ...
        if not merge:
            ...
            # Import here so that we don't need to depend on psycopg2 for Kalanfa in general.
...
class NoVersionChannelImport(NoLearningActivitiesChannelImport):
    ...
    from the old version of the Kalanfa content databases into the database for the current version of Kalanfa.
    """
...
# version of Kalanfa. When a new schema version is added
mappings = {
        ...
        try:
            ...
            if version_number > int(CONTENT_SCHEMA_VERSION):
                raise FutureSchemaError(
                    "Tried to import schema version, {version}, which is not supported by this version of Kalanfa.".format(
                        version=min_version
                    )
                )
...
def import_channel_by_id(
    ...
    try:
        ...
        raise KalanfaUpgradeError(
            "Database file uses a future database schema that this version of Kalanfa does not support."
        )
