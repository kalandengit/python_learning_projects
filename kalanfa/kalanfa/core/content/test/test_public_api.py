...
class ImportMetadataTestCase(APITestCase):
    ...
    def _assert_data(self, Model, queryset):
        response = self.client.get(
            reverse("kalanfa:core:importmetadata-detail", kwargs={"pk": self.node.id})
        )
    ...
    def test_import_metadata_none_pk(self):
        response = self.client.get(
            reverse(
                "kalanfa:core:importmetadata-detail",
                kwargs={"pk": None},
            )
        )
        self.assertEqual(response.status_code, 400)
    ...
    def test_import_metadata_with_descendants_returns_ancestors_and_descendants(self):
        ...
        response = self.client.get(
            reverse("kalanfa:core:importmetadata-detail", kwargs={"pk": self.root.id})
            + "?descendants=true"
        )
        self.assertEqual(response.status_code, 200)
