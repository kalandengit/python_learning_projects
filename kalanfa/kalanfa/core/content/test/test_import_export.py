...
import kalanfa.core.content.management.commands.importchannel as importchannel_module
from kalanfa.core.content.constants.transfer_types import COPY_METHOD
from kalanfa.core.content.errors import InsufficientStorageSpaceError
...
from kalanfa.core.content.tasks import remoteimport
from kalanfa.core.content.upgrade import populate_channel_library_field
...
from kalanfa.core.content.utils.import_export_content import get_import_export_data
from kalanfa.core.content.utils.import_export_content import get_import_export_nodes
from kalanfa.core.content.utils.paths import get_channel_lookup_url
from kalanfa.core.content.utils.resource_export import DiskChannelResourceExportManager
from kalanfa.core.content.utils.resource_import import DiskChannelResourceImportManager
from kalanfa.core.content.utils.resource_import import lookup_channel_listing_status
from kalanfa.core.content.utils.resource_import import (
    RemoteChannelDatabaseImportManager,
)
from kalanfa.core.content.utils.resource_import import (
    RemoteChannelResourceImportManager,
)
from kalanfa.core.device.models import ContentCacheKey
from kalanfa.core.discovery.utils.network.errors import NetworkLocationResponseFailure
from kalanfa.utils.file_transfer import Transfer
from kalanfa.utils.file_transfer import TransferCanceled
from kalanfa.utils.file_transfer import TransferFailed
...
class GetImportExportDataTestCase(TestCase):
    ...
    @patch("kalanfa.core.content.utils.import_export_content.get_import_export_nodes")
    @patch("kalanfa.core.content.utils.import_export_content.get_content_nodes_data")
...
class GetImportExportNodesTestCase(TestCase):
    ...
    @patch(
        "kalanfa.core.content.utils.import_export_content.get_channel_stats_from_disk"
    )
...
@patch("kalanfa.core.content.utils.channel_import.import_channel_from_local_db")
@patch(
    "kalanfa.core.content.management.commands.importchannel.AsyncCommand.start_progress"
)
@override_option("Paths", "CONTENT_DIR", tempfile.mkdtemp())
class ImportChannelTestCase(TestCase):
    ...
    @patch(
        "kalanfa.core.content.utils.channel_transfer.paths.get_content_database_file_url"
    )
    @patch(
        "kalanfa.core.content.utils.channel_transfer.paths.get_content_database_file_path"
    )
    @patch("kalanfa.core.content.utils.channel_transfer.transfer.FileDownload")
    @patch("kalanfa.core.content.utils.channel_transfer.get_current_job")
    ...
    @patch("kalanfa.core.content.utils.channel_transfer.transfer.FileCopy")
    @patch("kalanfa.core.content.utils.channel_transfer.get_current_job")
    ...
    @patch("kalanfa.utils.file_transfer.FileDownload._set_headers")
    @patch("kalanfa.core.content.utils.channel_transfer.get_current_job")
    def test_remote_import_sslerror(
        ...
        with patch(
            "kalanfa.utils.file_transfer.FileDownload._run_download",
    ...
    @patch("kalanfa.core.content.utils.channel_transfer.transfer.FileDownload")
    @patch("kalanfa.core.content.utils.channel_transfer.clear_channel_stats")
    ...
    @patch(
        "kalanfa.core.content.management.commands.importchannel.set_channel_metadata_fields"
    )
    @patch(
        "kalanfa.core.content.utils.channel_transfer.paths.get_content_database_file_url"
    )
    ...
    @patch("kalanfa.core.content.utils.channel_transfer.get_current_job")
    @patch(
        "kalanfa.core.content.management.commands.importchannel.lookup_channel_listing_status"
    )
    def test_network_import_with_token(
        ...
        mock_set_fields.assert_called_once_with(
            self.the_channel_id, library="KALANFA", version=5
        )
    ...
    @patch("kalanfa.core.content.management.commands.importchannel.transfer_channel")
    @patch(
        "kalanfa.core.content.management.commands.importchannel.lookup_channel_listing_status"
    )
    def test_token_resolution_uses_lookup_channel_listing_status(
        ...
        mock_set_fields.assert_called_once_with(
            the_channel_id, library="KALANFA", version=5
        )
...
@patch(
    "kalanfa.core.content.utils.resource_import.lookup_channel_listing_status",
)
class ChannelDbVersionTestCase(TestCase):
    ...
    @patch("kalanfa.core.content.utils.resource_import.transfer.FileDownload")
    @patch(
        "kalanfa.core.content.utils.resource_import.paths.get_content_database_file_url",
    ...
    @patch(
        "kalanfa.core.content.utils.resource_import.SameHostSession",
    )
    @patch(
        "kalanfa.core.content.utils.resource_import.paths.get_content_database_file_url",
        return_value="http://test/channel.db",
...
@patch("kalanfa.core.content.utils.resource_import.get_import_export_data")
@override_option("Paths", "CONTENT_DIR", tempfile.mkdtemp())
class ImportContentTestCase(TestCase):
    ...
    def setUp(self):
        ...
        patcher = patch("kalanfa.core.content.utils.resource_import.annotation")
    ...
    @patch("kalanfa.core.content.utils.resource_import.transfer.FileDownload")
    @patch(
        "kalanfa.core.content.utils.resource_import.JobProgressMixin.check_for_cancel"
    )
    @patch(
        "kalanfa.core.content.utils.resource_import.JobProgressMixin.is_cancelled",
        return_value=True,
    )
    ...
    @patch(
        "kalanfa.core.content.utils.resource_import.paths.get_content_storage_remote_url"
    )
    @patch(
        "kalanfa.core.content.utils.resource_import.paths.get_content_storage_file_path"
    )
    @patch("kalanfa.core.content.utils.resource_import.transfer.FileDownload")
    @patch(
        "kalanfa.core.content.utils.resource_import.JobProgressMixin.check_for_cancel"
    ...
    @patch(
        "kalanfa.core.content.utils.resource_import.transfer.Transfer._checksum_correct",
        return_value=True,
    )
    @patch(
        "kalanfa.core.content.utils.resource_import.paths.get_content_storage_remote_url"
    ...
    @patch("kalanfa.core.content.utils.resource_import.transfer.FileCopy")
    @patch(
        "kalanfa.core.content.utils.resource_import.JobProgressMixin.check_for_cancel"
    )
    @patch(
    ...
    @patch("kalanfa.core.content.utils.resource_import.logger.warning")
    @patch(
        "kalanfa.core.content.utils.resource_import.paths.get_content_storage_file_path"
    )
    def test_remote_import_httperror_404(
        ...
        with patch(
            "kalanfa.core.content.utils.resource_import.transfer.FileDownload.run",
    ...
    @patch("kalanfa.core.content.utils.resource_import.get_free_space")
    @patch("kalanfa.core.content.utils.resource_import.transfer.FileDownload.finalize")
    @patch(
        "kalanfa.core.content.utils.resource_import.paths.get_content_storage_file_path"
    )
    @patch(
    ...
    def test_remote_import_no_space_after_first_download(
        ...
        with patch(
            "kalanfa.core.tasks.utils.get_fd_limit", return_value=1
    ...
    @patch("kalanfa.utils.file_transfer.sleep")
    @patch(
        "kalanfa.utils.file_transfer.FileDownload._run_download",
    ...
    @patch("kalanfa.core.content.utils.resource_import.logger.error")
    @patch("kalanfa.utils.file_transfer.os.path.getsize")
    @patch(
        "kalanfa.core.content.utils.resource_import.paths.get_content_storage_file_path"
    )
    ...
    @patch("kalanfa.core.content.utils.resource_import.transfer.os.remove")
    @patch(
        "kalanfa.core.content.utils.resource_import.os.path.isfile",
        return_value=False,
    )
    @patch(
        "kalanfa.core.content.utils.resource_import.paths.get_content_storage_file_path"
    ...
    @patch(
        "kalanfa.core.content.utils.resource_import.transfer.FileCopy._checksum_correct",
        return_value=True,
    )
    ...
    @patch(
        "kalanfa.core.content.utils.resource_import.transfer.FileDownload._checksum_correct",
        return_value=False,
    )
    ...
    def test_remote_import_full_import(
        ...
        manager = RemoteChannelResourceImportManager(self.the_channel_id)
        # Ensure single threaded operation for deterministic testing
        with patch("kalanfa.core.tasks.utils.get_fd_limit", return_value=1):
            manager.run()
        self.annotation_mock.set_content_visibility.assert_called_with(
            self.the_channel_id,
            [
    ...
    @patch("kalanfa.core.content.utils.content_manifest.logger.warning")
    ...
    @patch("kalanfa.core.content.utils.resource_import.transfer.FileCopy")
    @patch(
        "kalanfa.core.content.utils.resource_import.os.path.getsize",
        return_value=1,
    )
    @patch(
        "kalanfa.core.content.utils.paths.existing_file_path_in_content_fallback_dirs",
...
class ExportChannelTestCase(TestCase):
    ...
    @patch(
        "kalanfa.core.content.utils.resource_export.paths.get_content_database_file_path"
    )
    @patch("kalanfa.core.content.utils.resource_export.transfer.FileCopy")
    @patch("kalanfa.core.content.utils.resource_export.JobProgressMixin.is_cancelled")
    @patch(
        "kalanfa.core.content.utils.resource_export.JobProgressMixin.check_for_cancel"
    )
...
@override_option("Paths", "CONTENT_DIR", tempfile.mkdtemp())
@patch("kalanfa.core.content.utils.resource_export.get_import_export_nodes")
@patch("kalanfa.core.content.utils.resource_export.get_content_nodes_data")
@patch("kalanfa.core.content.utils.resource_export.ContentManifest")
class ExportContentTestCase(TestCase):
    ...
    @patch(
        "kalanfa.core.content.utils.resource_export.paths.get_content_storage_file_path"
    )
    @patch("kalanfa.core.content.utils.resource_export.transfer.FileCopy")
    @patch(
        "kalanfa.core.content.utils.resource_export.JobProgressMixin.is_cancelled",
    ...
    @patch(
        "kalanfa.core.content.utils.resource_export.DiskChannelResourceExportManager._copy_content_files"
    )
    @patch(
        "kalanfa.core.content.utils.resource_export.JobProgressMixin.is_cancelled",
        return_value=False,
...
class TestFilesToTransfer(TestCase):
    ...
    @patch(
        "kalanfa.core.content.utils.import_export_content.get_channel_stats_from_peer"
    )
...
class DiskChannelResourceExportManagerTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.patches = [
            ...
            patch("kalanfa.core.content.utils.resource_export.transfer.FileCopy"),
            patch(
                "kalanfa.core.content.utils.resource_export.JobProgressMixin.start_progress"
            ),
            patch(
                "kalanfa.core.content.utils.resource_export.JobProgressMixin.update_progress"
            ),
        ]
...
class ImportManagerWithChannelDatabaseTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.patches = [
            patch("kalanfa.core.content.utils.resource_import.annotation"),
            patch(
                "kalanfa.core.content.utils.resource_import.lookup_channel_listing_status"
            ),
            patch(
                "kalanfa.core.content.utils.resource_import.JobProgressMixin.start_progress"
            ),
            patch(
                "kalanfa.core.content.utils.resource_import.JobProgressMixin.is_cancelled",
                return_value=False,
    ...
    @patch(
        "kalanfa.core.content.utils.resource_import.paths.get_content_database_file_path"
    )
    @patch("kalanfa.core.content.utils.resource_import.transfer.FileDownload")
    @patch("kalanfa.core.content.utils.resource_import.import_channel_by_id")
    ...
    def test_token_metadata_stored_on_channel_metadata(self):
        ...
        self.assertEqual(manager.library, "KALANFA")
...
class UpgradeDBReuseTestCase(TestCase):
    ...
    def setUp(self):
        self.patches = [
            ...
            patch(
                "kalanfa.core.content.utils.resource_import.import_channel_by_id",
                return_value=True,
            ),
        ]
...
class LookupChannelListingStatusTest(TestCase):
    ...
    @patch(
        "kalanfa.core.content.utils.resource_import.NetworkClient.discover_from_address"
    )
    def test_returns_dict_with_all_fields(self, mock_build):
        ...
        self.assertEqual(result["library"], "KALANFA")
...
class ChannelMetadataLibraryFieldTest(TestCase):
    ...
    def test_library_field_accepts_kalanfa_value(self):
        ChannelMetadata.objects.filter(id=self.the_channel_id).update(
            library=library_constants.KALANFA
        )
        channel = ChannelMetadata.objects.get(id=self.the_channel_id)
        self.assertEqual(channel.library, "KALANFA")
...
class SetChannelMetadataFieldsTest(TestCase):
    ...
    def test_sets_library_field(self):
        set_channel_metadata_fields(self.the_channel_id, library="KALANFA")
        channel = ChannelMetadata.objects.get(id=self.the_channel_id)
        self.assertEqual(channel.library, "KALANFA")
...
class PopulateChannelLibraryUpgradeTest(TestCase):
    ...
    def test_public_channel_gets_kalanfa_library(self):
        ChannelMetadata.objects.filter(id=self.the_channel_id).update(
