...
class Command(BaseCommand):
    ...
    def move_content_directory(self, dst):
        ...
        # Check if Kalanfa is running
        try:
            server.get_status()
            self.stderr.write(
                self.style.ERROR(
                    "Cannot migrate content while Kalanfa is running, please run:\n"
                    "\n"
                    "    kalanfa stop\n"
                )
            )
    ...
    def update_config_content_directory(self, dst):
        """
        Update kalanfa_settings.json in KALANFA_HOME so that the variable
        CONTENT_DIRECTORY points to the destination content directory.
        """
        ini_path = os.path.join(KALANFA_HOME, "options.ini")
