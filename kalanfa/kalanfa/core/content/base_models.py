...
models that are used within Kalanfa to track content metadata
and are also used within Kalanfa Studio to prepare content metadata
for export
...
Specifically, Kalanfa Studio has a modified version of this models.py for generating
...
However, in the case where the modifications are intended only within Kalanfa, such
...
the command can be run without arguments, and it will auto increment:
    ...
    `kalanfa manage generate_schema`
...
passed to the command should be the value of the new version e.g. VERSION_3:
    ...
    `kalanfa manage generate_schema 3`
...
kalanfa/core/content/constants/schema_versions.py must be
...
class AssessmentMetaData(models.Model):
    """
    A model to describe additional metadata that characterizes assessment behaviour in Kalanfa.
...
class ChannelMetadata(models.Model):
    ...
    # Minimum version of Kalanfa that this content database is compatible with
