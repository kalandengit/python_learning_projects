...
from kalanfa.core.device.tasks import DeviceProvisionValidator
from kalanfa.core.device.tasks import provisiondevice
from kalanfa.core.device.utils import APP_AUTH_TOKEN_COOKIE_NAME
from kalanfa.core.device.utils import app_initialize_url
from kalanfa.core.device.utils import APP_KEY_COOKIE_NAME
from kalanfa.plugins.utils.test.helpers import plugin_disabled
...
class DeviceProvisionTestCase(TestCase):
    ...
    def _default_provision_data(self):
        return {
            "type": "kalanfa.core.device.tasks.provisiondevice",
            "device_name": None,
    ...
    def _post_deviceprovision(self, data, auth_token=None):
        request = None
        if auth_token:
            from kalanfa.core.device.models import DeviceAppKey
    ...
    def test_osuser_superuser_error_no_app(self):
        with plugin_disabled("kalanfa.plugins.app"):
            data = self._default_provision_data()
            del data["superuser"]
            with pytest.raises(ValidationError):
                self._post_deviceprovision(data)
    ...
    def test_osuser_superuser_created(self):
        with mock.patch(
            "kalanfa.core.device.tasks.GetOSUserHook", autospec=True
        ) as mock_hook1, mock.patch(
            "kalanfa.core.auth.models.GetOSUserHook"
        ) as mock_hook2:
