...
from kalanfa.core.content.tasks import automatic_resource_import
from kalanfa.core.content.tasks import automatic_synchronize_content_requests_and_import
...
class DeviceSerializerMixin:
    def validate_language_id(self, language_id):
        """
        Check that the language_id is supported by Kalanfa
        """
        if language_id is not None and not check_for_language(language_id):
            raise serializers.ValidationError(_("Language is not supported by Kalanfa"))
        return language_id
