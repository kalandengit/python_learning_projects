...
from kalanfa.core.webpack.hooks import WebpackInclusionSyncMixin
from kalanfa.plugins.hooks import define_hook
from kalanfa.plugins.hooks import KalanfaHook
from kalanfa.plugins.utils import plugin_url
...
@define_hook
class RoleBasedRedirectHook(KalanfaHook):
...
class FrontEndBaseSyncHook(WebpackInclusionSyncMixin):
    """
    Inherit a hook defining assets to be loaded in kalanfa/base.html, that means
    ALL pages. Use with care.
    """


@define_hook
class FrontEndBaseHeadHook(KalanfaHook):
    """
    Inherit a hook defining markup to be injected in the head of
    kalanfa/base.html, that means ALL pages. Use with care.
    """
...
class LogoutRedirectHook(KalanfaHook):
    """
