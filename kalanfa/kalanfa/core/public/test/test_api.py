...
class PublicAPITestCase(APITestCase):
    ...
    def test_info_endpoint_unversioned(self):
        response = self.client.get(reverse("kalanfa:core:info-list"))
        instance_model = InstanceIDModel.get_or_create_current_instance()[0]
        settings = DeviceSettings.objects.get()
        self.assertEqual(response.data["application"], "kalanfa")
        self.assertEqual(response.data["kalanfa_version"], kalanfa.__version__)
    ...
    def test_info_endpoint_v1(self):
        response = self.client.get(reverse("kalanfa:core:info-list"), data={"v": "1"})
        instance_model = InstanceIDModel.get_or_create_current_instance()[0]
        settings = DeviceSettings.objects.get()
        self.assertEqual(response.data["application"], "kalanfa")
        self.assertEqual(response.data["kalanfa_version"], kalanfa.__version__)
    ...
    def test_info_endpoint_v2(self):
        response = self.client.get(reverse("kalanfa:core:info-list"), data={"v": "2"})
        instance_model = InstanceIDModel.get_or_create_current_instance()[0]
        settings = DeviceSettings.objects.get()
        self.assertEqual(response.data["application"], "kalanfa")
        self.assertEqual(response.data["kalanfa_version"], kalanfa.__version__)
    ...
    def test_public_checksum_lookup_no_checksums(self):
        response = self.client.post(
            reverse("kalanfa:core:get_public_file_checksums", kwargs={"version": "v1"}),
            data=[],
            format="json",
        )
...
class SyncQueueViewSetTestCase(APITestCase):
    ...
    @mock.patch(
        "kalanfa.core.public.api.get_device_setting",
        return_value=True,
    )
    def test_soud(self, mock_device_setting):
        response = self.client.post(
            reverse("kalanfa:core:syncqueue"),
