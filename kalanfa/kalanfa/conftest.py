...
TEMP_KALANFA_HOME = "./.pytest_kalanfa_home"
...
# see kalanfa/core/discovery/test/helpers.py for reusable helpers.
...
def _block(api, host, port):
    raise BlockedNetworkAccessError(
        ...
        "(see kalanfa/core/discovery/test/helpers.py).".format(
...
def _use_small_test_keys():
    ...
    # Kalanfa ships, each keygen costs ~1-2s and dominates test setup. Key
...
def global_fixture():
    if not os.path.exists(TEMP_KALANFA_HOME):
        os.mkdir(TEMP_KALANFA_HOME)
    if not os.path.exists(os.path.join(TEMP_KALANFA_HOME, "content")):
        os.mkdir(os.path.join(TEMP_KALANFA_HOME, "content"))
    yield  # wait until the test ended
    if os.path.exists(TEMP_KALANFA_HOME):
        try:
            shutil.rmtree(TEMP_KALANFA_HOME)
        except OSError:
