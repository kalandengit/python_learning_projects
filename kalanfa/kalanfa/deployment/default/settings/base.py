"""
Django settings for kalanfa project.
...
from kalanfa.deployment.default.sqlite_db_names import JOB_STORAGE
from kalanfa.plugins.utils.settings import apply_settings
...
if not os.path.exists(conf.KALANFA_HOME):
    raise RuntimeError("The KALANFA_HOME dir does not exist")

# Build paths inside the project like this: os.path.join(BASE_DIR, ...)
# import kalanfa, so we can get the path to the module.
# we load other utilities related to i18n
# This is essential! We load the kalanfa conf INSIDE the Django conf

KALANFA_MODULE_PATH = os.path.dirname(kalanfa.__file__)

BASE_DIR = os.path.abspath(os.path.dirname(__name__))

LOCALE_PATHS = [os.path.join(KALANFA_MODULE_PATH, "locale")]
...
INSTALLED_APPS = [
    ...
    "django_filters",
    "kalanfa.core.auth.apps.KalanfaAuthConfig",
    "kalanfa.core.bookmarks",
    "kalanfa.core.content",
    "kalanfa.core.logger",
    "kalanfa.core.notifications.apps.KalanfaNotificationsConfig",
    "kalanfa.core.tasks.apps.KalanfaTasksConfig",
    "kalanfa.core.deviceadmin",
    "kalanfa.core.webpack",
    "kalanfa.core.exams",
    "kalanfa.core.courses",
    "kalanfa.core.device",
    "kalanfa.core.discovery",
    "kalanfa.core.lessons",
    "kalanfa.core.analytics",
    "kalanfa.core.attendance",
    "rest_framework",
    "django_js_reverse",
    "jsonfield",
    "morango",
...
MIDDLEWARE = [
    "kalanfa.core.analytics.middleware.cherrypy_access_log_middleware",
    "kalanfa.core.device.middleware.ProvisioningErrorHandler",
    "kalanfa.core.device.middleware.DatabaseBusyErrorHandler",
    "django.middleware.cache.UpdateCacheMiddleware",
    "kalanfa.core.analytics.middleware.MetricsMiddleware",
    "kalanfa.core.auth.middleware.KalanfaSessionMiddleware",
    "kalanfa.core.device.middleware.KalanfaLocaleMiddleware",
    "django.middleware.common.CommonMiddleware",
    "csp.middleware.CSPMiddleware",
    "django.middleware.csrf.CsrfViewMiddleware",
    "kalanfa.core.auth.middleware.CustomAuthenticationMiddleware",
...
ROOT_URLCONF = "kalanfa.deployment.default.urls"
...
TEMPLATES = [
    {
        ...
        "OPTIONS": {
            "context_processors": [
                ...
                "kalanfa.core.context_processors.custom_context_processor.developer_mode",
            ]
        },
    }
]

WSGI_APPLICATION = "kalanfa.deployment.default.wsgi.application"
...
if conf.OPTIONS["Database"]["DATABASE_ENGINE"] == "sqlite":
    ...
    DATABASES = {
        "default": {
            "ENGINE": "kalanfa.deployment.default.db.backends.sqlite3",
    ...
    for additional_db in ADDITIONAL_SQLITE_DATABASES:
        ...
        if additional_db == JOB_STORAGE:
            # Override test config for job storage to use a sqlite file in KALANFA_HOME rather than in-memory
            # as jobs tasks may be run in a separate thread/process
            db_config["TEST"] = {
                "NAME": os.path.join(conf.KALANFA_HOME, "test_job_storage.sqlite3")
            }

        DATABASES[additional_db] = db_config

    DATABASE_ROUTERS = (
        "kalanfa.core.auth.models.SessionRouter",
        "kalanfa.core.notifications.models.NotificationsRouter",
        "kalanfa.core.device.models.SyncQueueRouter",
        "kalanfa.core.discovery.models.NetworkLocationRouter",
        "kalanfa.core.tasks.models.KalanfaTasksRouter",
    )
...
LANGUAGES = [
    (
        i18n.KALANFA_LANGUAGE_INFO[lang_code]["intl_code"],
        i18n.KALANFA_LANGUAGE_INFO[lang_code]["language_name"],
    )
    for lang_code in conf.OPTIONS["Deployment"]["LANGUAGES"]
    if lang_code in i18n.KALANFA_LANGUAGE_INFO
]
...
STATIC_ROOT = os.path.join(conf.KALANFA_HOME, "static")
MEDIA_URL = urljoin(path_prefix, "media/")
MEDIA_ROOT = os.path.join(conf.KALANFA_HOME, "media")
...
CSRF_COOKIE_NAME = "kalanfa_csrftoken"
...
AUTH_USER_MODEL = "kalanfaauth.FacilityUser"

# Our own custom setting to override the anonymous user model

AUTH_ANONYMOUS_USER_MODEL = "kalanfaauth.KalanfaAnonymousUser"

AUTHENTICATION_BACKENDS = ["kalanfa.core.auth.backends.FacilityUserBackend"]
...
REST_FRAMEWORK = {
    ...
    "DEFAULT_CONTENT_NEGOTIATION_CLASS": "kalanfa.core.negotiation.LimitContentNegotiation",
...
SESSION_ENGINE = "kalanfa.core.auth.backends"

SECURE_CONTENT_TYPE_NOSNIFF = True

SESSION_COOKIE_NAME = "kalanfa"
...
MORANGO_INSTANCE_INFO = os.environ.get(
    ...
    "kalanfa.core.auth.constants.morango_sync:CUSTOM_INSTANCE_INFO",
)
# prepend our own Morango Operation to handle custom behaviors during sync
SYNC_OPERATIONS = ("kalanfa.core.auth.sync_operations:KalanfaSyncOperations",)
...
# whether Kalanfa is running within tests
TESTING = False
