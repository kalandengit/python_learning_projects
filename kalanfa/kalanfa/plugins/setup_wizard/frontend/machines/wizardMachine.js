...
import { getImportLodUsersDefinition } from 'kalanfa-common/machines/importLodUsersMachine';
...
export const wizardMachine = createMachine(
  {
    ...
    on: {
      START_OVER: { target: 'howAreYouUsingKalanfa', actions: 'resetContext' },
    },
    states: {
      ...
      initializeContext: {
        on: {
          CONTINUE: { target: 'howAreYouUsingKalanfa', actions: 'setCanGetOsUser' },
        },
      },
      // Initial step where user selects between "On my own" or "Group learning"
      howAreYouUsingKalanfa: {
        meta: { route: { name: 'HOW_ARE_YOU_USING_KALANFA', path: '/' } },
        on: {
      ...
      importLodUsers: {
        initial: importLodUsersDefinition.initial,
        states: importLodUsersDefinition.states,
        on: {
  ...
  {
    actions: {
      ...importLodUsersDefinition.actions,
