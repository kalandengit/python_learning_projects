...
<script>
  ...
  import { error as coreError, handleApiError, clearError } from 'kalanfa/utils/appError';
  import pluginData from 'kalanfa-plugin-data';
  import AppError from 'kalanfa/components/error/AppError';
  ...
  import KalanfaLoadingSnippet from 'kalanfa-common/components/KalanfaLoadingSnippet';
  import commonCoreStrings from 'kalanfa/uiText/commonCoreStrings';
  import { Presets } from 'kalanfa/constants';
  ...
  export default {
    name: 'SettingUpKalanfa',
    components: { AppError, KalanfaLoadingSnippet },
    ...
    $trs: {
      pageTitle: {
        message: 'Setting up Kalanfa',
        context: 'The title of the page',
      },
      pleaseWaitMessage: {
        message: 'This may take several minutes',
        context: 'Kalanfa is working in the background and the user may need to wait',
      },
      onMyOwnDeviceName: {
        message: 'Personal device for {name}',
        context:
          "The default device name for a user installing Kalanfa using the personal 'on my own' (formerly Quick Start) flow",
      },
      onMyOwnFacilityName: {
        message: 'Home Facility for {name}',
        context:
          "Default facility name when Kalanfa is installed with the 'Quick start' setup option for at home learning, outside any type of structure or institution like a school or a library. '{name}' will display the full name of the super admin user for their Kalanfa server. Note that users can change this default name after the setup, and put whatever name they want to use for their home facility.",
      },
    },
  };
