...
class PwaPlugin(KalanfaPluginBase):
    """
    A plugin to support presenting the Kalanfa instance as a Progressive Web App
    (PWA).

    This will export a [Web Application Manifest](https://www.w3.org/TR/appmanifest/)
    for the Kalanfa instance, so that users can easily run it in a standalone
    browser window.

    For this to give a good user experience, Kalanfa must be available locally
    or when the user is offline. The current PWA service worker will not cache
    anything more than the browser usually does.

    For the web app to be installable, the Kalanfa instance must be set up to be
