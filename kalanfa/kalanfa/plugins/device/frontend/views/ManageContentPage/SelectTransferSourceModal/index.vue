    ...
    methods: {
      ...
      nextState(data) {
        if (this.atSelectImportSource) {
          if (data.source === ContentSources.KALANFA_STUDIO) {
            this.$emit('submit', { source: ContentSources.KALANFA_STUDIO });
          } else if (data.source === ContentSources.LOCAL_DRIVE) {
            this.$emit('update:pageName', ContentWizardPages.SELECT_DRIVE);
          } else if (data.source === ContentSources.PEER_KALANFA_SERVER) {
          ...
          this.$emit('submit', {
            source: ContentSources.PEER_KALANFA_SERVER,
