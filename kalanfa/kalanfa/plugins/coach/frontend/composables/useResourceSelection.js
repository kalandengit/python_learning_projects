...
import useBaseSearch from 'kalanfa-common/composables/useBaseSearch';
import useFetch from 'kalanfa-common/composables/useFetch.js';

/**
 * @typedef {import('kalanfa-common/composables/useFetch.js').FetchObject} FetchObject
 */

/**
 * @typedef {object} UseResourceSelectionResponse
 * @property {object} topic - Topic tree object; contains the information of the topic, its
 * ascendants and children. Defined only if the `topicId` query in the route is set.
 ...
 * @property {FetchObject} searchFetch - Search fetch object to manage the process of fetching
 * search results. Fetching more search results is supported.
 * @property {Array<string>} searchTerms - The search terms used to filter the search results.
 * @property {boolean} displayingSearchResults - Indicates whether we currently have search
 * terms.
 * @property {Array<(node: object) => boolean>} selectionRules - An array of functions that
 * determine whether a node can be selected.
 ...
 * @property {(resources: Array<object>) => void} deselectResources - Removes the specified
 * resources from the `selectedResources` array.
 ...
 * `filters` an object with extra query params, and `annotator` a function to annotate the
 * results.
 ...
 * `filters` an object with extra query params, and `annotator` a function to annotate the
 * results.
 * @param {string} options.searchResultsRouteName - The name of the route where the search
 * results will be displayed so that we can redirect to it when the search terms are updated.
 ...
 * @returns {UseResourceSelectionResponse} Resource selection state and methods.
 */
