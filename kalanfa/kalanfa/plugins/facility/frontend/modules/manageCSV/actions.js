...
import GenerateCSVLogRequestResource from 'kalanfa-common/apiResources/GenerateCSVLogRequestResource';
...
function getFirstLogDate(store) {
  return client({
    url: urls['kalanfa:kalanfa.plugins.facility:firstlogdate'](store.rootGetters.activeFacilityId),
  }).then(response => {
...
function getExportedCSVsInfo(store) {
  return client({
    url: urls['kalanfa:kalanfa.plugins.facility:exportedcsvinfo'](
