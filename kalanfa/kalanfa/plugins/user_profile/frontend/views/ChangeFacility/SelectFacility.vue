...
<script>
  ...
  import useDevices from 'kalanfa-common/components/syncComponentSet/SelectDeviceModalGroup/useDevices';
  import useDeviceDeletion from 'kalanfa-common/components/syncComponentSet/SelectDeviceModalGroup/useDeviceDeletion';
  ...
  export default {
    ...
    setup() {
      ...
      const storageFacilityId = useLocalStorage('kalanfa-lastSelectedFacilityId', '');
          ...
          try {
            ...
            return facilities.map(facility => {
              return {
                ...
                kalanfa_version: device.kalanfa_version,
              };
            });
          } catch (e) {
            return [];
      ...
      const facilityDisabled = computed(() => {
        return function (facility) {
          return (
            ...
            !isMinimumKalanfaVersion(facility.kalanfa_version)
          );
        };
      });
      ...
      function resetSelectedAddress() {
        const enabledFacilities = availableFacilities.value.filter(f =>
          isMinimumKalanfaVersion(f.kalanfa_version),
        );
