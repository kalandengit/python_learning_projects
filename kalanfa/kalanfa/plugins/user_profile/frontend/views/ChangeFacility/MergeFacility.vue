...
<script>
  ...
  import { syncStatusToDescriptionMap, TaskStatuses } from 'kalanfa-common/utils/syncTaskUtils';
  ...
  export default {
    ...
    setup() {
      ...
      function syncFacilityTaskDisplayInfo(task) {
        // overrides the syncFacilityTaskDisplayInfo function in kalanfa.utils.syncTaskUtils
          ...
          TaskResource.fetchCollection()
            .then(allTasks => {
              const tasks = allTasks.filter(
                t => t.type === 'kalanfa.plugins.user_profile.tasks.mergeuser',
              );
              if (tasks.length > 0) {
      ...
      function to_finish() {
        ...
        client({
          url: urls['kalanfa:kalanfa.plugins.user_profile:loginmergeduser'](),
          method: 'POST',
          data: params,
        }).then(response => {
          if (
