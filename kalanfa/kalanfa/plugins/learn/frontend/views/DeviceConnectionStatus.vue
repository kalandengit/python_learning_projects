...
<script>
  ...
  export default {
    ...
    setup(props) {
      ...
      // about special static locations like Kalanfa Studio and KDP, we need to
      // do a separate check for those.
      // Once that functionality has been enabled, this can be cleaned up!
      if (props.deviceId === KalanfaStudioId) {
        const isFetched = ref(false);
        const allDevices = ref([]);
        const getStudio = async () => {
          const response = await RemoteChannelResource.getKalanfaStudioStatus();
          set(allDevices, [
            {
              ...response.data,
              id: KalanfaStudioId,
              instance_id: KalanfaStudioId,
            },
          ]);
