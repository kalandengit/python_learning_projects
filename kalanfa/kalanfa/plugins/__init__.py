...
from kalanfa.utils.build_config.default_plugins import DEFAULT_PLUGINS
from kalanfa.utils.conf import KALANFA_HOME

logger = logging.getLogger(__name__)


conf_file = os.path.join(KALANFA_HOME, "plugins.json")
...
class ConfigDict(dict):
    ...
    def __init__(self):
        ...
        if os.path.isfile(conf_file):
            try:
                ...
                with open(conf_file, "r") as kalanfa_conf_file:
                    self.update(json.load(kalanfa_conf_file))
                return
        ...
        self.update(
            {
                ...
                "DISABLED_PLUGINS": [],
                # Plugins that have been updated since we last initialized Kalanfa
                "UPDATED_PLUGINS": [],
    ...
    def ENV_VAR_APPLIED_PLUGINS(self):
        return [
            p.strip()
            for p in os.environ.get("KALANFA_PLUGIN_APPLY", "").split(",")
            if p.strip()
        ]
    ...
    def ENV_VAR_ENABLED_PLUGINS(self):
        return set(
            p.strip()
            for p in os.environ.get("KALANFA_PLUGIN_ENABLE", "").split(",")
            if p.strip()
        )
    ...
    def ENV_VAR_DISABLED_PLUGINS(self):
        return set(
            p.strip()
            for p in os.environ.get("KALANFA_PLUGIN_DISABLE", "").split(",")
            if p.strip()
        )
    ...
    def save(self):
        ...
        with open(conf_file, "w") as kalanfa_conf_file:
            json.dump(config_copy, kalanfa_conf_file, indent=2, sort_keys=True)
...
class KalanfaPluginBase(metaclass=SingletonMeta):
    """
    This is the base class that all Kalanfa plugins need to implement.
    """
    ...
    def _installed_apps_add(self):
        ...
        added to Kalanfa configuration"""
    ...
    def enable(self):
        """Modify the kalanfa config dict to your plugin's needs"""
    ...
    def url_module(self):
        ...
        this::
            ...
            reverse('kalanfa:mypluginclass:url_name')
    ...
    def options_module(self):
        ...
        By default this will be discovered based on the kalanfa_options
        property.
        """
        if self.kalanfa_options:
            module = self._return_module(self.kalanfa_options)
            if module is None:
                logging.warning(
                    "{plugin} defined {module} kalanfa options but the module was not found".format(
                        plugin=self.module_path, module=self.kalanfa_options
                    )
                )
            return module
    ...
    def option_defaults_module(self):
        ...
        property.
        """
        if self.kalanfa_option_defaults:
            module = self._return_module(self.kalanfa_option_defaults)
            if module is None:
                logging.warning(
                    "{plugin} defined {module} kalanfa option defaults but the module was not found".format(
                        plugin=self.module_path, module=self.kalanfa_option_defaults
                    )
                )
            return module
