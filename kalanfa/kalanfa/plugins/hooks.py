...
"hook into a component" in Kalanfa and have it do a predefined and
parameterized *thing*. For instance, Kalanfa could ask all its plugins who
...
The consequences of a hook being applied can happen anywhere in Kalanfa. Each
hook is defined through a class inheriting from ``KalanfaHook``. But how the
...
All Kalanfa core applications and plugins alike should *by convention* define
their abstract hooks inside ``<myapp>/hooks.py``. Thus, to see which hooks
a Kalanfa component exposes, you can refer to its ``hooks`` module.

.. note::
    Defining abstract hooks in ``<myapp>/hooks.py`` isn't mandatory, but
    *loading* a concrete hook in ``<myapp>/kalanfa_plugin.py`` is.

.. warning::
    Do not define abstract and registered hooks in the same module. Or to put it
    in other words: Always put registered hooks in ``<myapp>/kalanfa_plugin.py``. The
    registered hooks will only be initialized for use by the Kalanfa plugin registry
    if they are registered inside the kalanfa_plugin.py module for the plugin.
...
.. note::
    ...
    readability. The actual implementation in Kalanfa will differ.


Example implementation
----------------------

Here is an example of how to use a hook in ``myplugin.kalanfa_plugin.py``:
...
against any abstract KalanfaHook descendants that it inherits from. In this case, the hook
...
Here is the definition of the abstract NavigationHook in kalanfa.core.hooks:
...
of the ``NavigationHook`` in ``kalanfa/core/kalanfa_plugin.py``:
...
.. warning::
    ...
    ``kalanfa_plugin``. Either define them there directly or import the modules
    that define them. Hook classes should all be seen at load time, and
    placing that logic in ``kalanfa_plugin`` guarantees that things are
...
from kalanfa.plugins import SingletonMeta
...
def _add_kalanfa_hook_meta(subclass):
    """
    Add the KalanfaHookMeta metaclass to the subclass.
    """
    ...
    return KalanfaHookMeta(subclass.__name__, subclass.__bases__, orig_vars)
...
def define_hook(subclass=None, only_one_registered=False):
    ...
    subclass = _add_kalanfa_hook_meta(subclass)
...
def register_hook(subclass):
    ...
    if not any(
        ...
        and base.abstract
        and issubclass(base, KalanfaHook)
        for base in subclass.__bases__
    ):
        raise TypeError(
            "register_hook decorator used on a class that does not inherit from any abstract KalanfaHook subclasses"
        )
    if not subclass.__module__.endswith("kalanfa_plugin"):
        raise RuntimeError(
            "register_hook decorator invoked outside of a kalanfa_plugin.py module - this hook will not be initialized"
        )
...
def unregister_hook(subclass):
    ...
    if not any(
        ...
        raise TypeError(
            "unregister_hook decorator used on a class that does not inherit from any abstract KalanfaHook subclasses"
        )
    if not subclass.__module__.endswith("kalanfa_plugin"):
        raise RuntimeError(
            "unregister_hook decorator invoked outside of a kalanfa_plugin.py module - this hook will not be initialized"
        )
    subclass.remove_hook_from_registries()
    subclass._registered = False


class KalanfaHookMeta(SingletonMeta):
    """
    We use a metaclass to define class level properties and methods in a simple way.
    We could define the classmethods on the KalanfaHook object below, but this keeps
    ...
    def add_hook_to_registries(cls):
        ...
        if not cls.abstract and cls._registered:
            ...
            for parent in cls.__mro__:
                if (
                    isabstract(parent)
                    and issubclass(parent, KalanfaHook)
                    and parent is not KalanfaHook
...
class KalanfaHook(metaclass=KalanfaHookMeta):
    @property
    @abstractmethod
