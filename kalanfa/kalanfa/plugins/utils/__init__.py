...
from kalanfa.plugins import conf_file
from kalanfa.plugins import config
from kalanfa.plugins import ConfigDict
...
from kalanfa.utils.version import normalize_version_to_semver
...
class PluginLoadsApp(Exception):
    """
    An exception raised in case a kalanfa_plugin.py results in loading of the
    Django app stack.
    """
...
class MultiplePlugins(Exception):
    ...
    kalanfa_plugin module.
    """


def plugin_url(plugin_class, url_name):
    return reverse(
        "kalanfa:{namespace}:{url_name}".format(
...
def _is_plugin(class_definition):
    return isinstance(class_definition, type) and issubclass(
        class_definition, KalanfaPluginBase
    )
...
def is_external_plugin(module_path):
    ...
    not prefixed with "kalanfa.". If so, we know it's not an internal plugin.
    """

    return not module_path.startswith("kalanfa.")
...
def initialize_plugins_and_hooks(all_classes, plugin_name, initialize_hooks=True):
    ...
    for class_definition in all_classes:
        ...
        elif issubclass(class_definition, KalanfaHook) and initialize_hooks:
    ...
    if len(plugin_objects) == 0:
        raise PluginDoesNotExist(
            "Plugin '{}' exists but does not define a KalanfaPluginBase derived class".format(
                plugin_name
            )
        )
    ...
    if len(plugin_objects) > 1:
        raise MultiplePlugins("More than one plugin defined in kalanfa_plugin module")


def initialize_kalanfa_plugin(plugin_name, initialize_hooks=True):
    """
    Try to load kalanfa_plugin from given plugin module identifier
    In so doing, it will instantiate the KalanfaPlugin object if it
    exists, and also register any hooks found in the module.

    Set the initialize_hooks argument to False to just retrieve the kalanfa plugin without registering
    its hooks.

    :returns: the KalanfaPlugin object for the module
    """
    was_configured = django_settings.configured
    ...
    plugin_module_name = plugin_name + ".kalanfa_plugin"
    try:
        ...
        logger.debug("Loaded kalanfa plugin: {}".format(plugin_name))
        ...
        def is_plugin_module(x):
            return (
                ...
                and plugin_package + ".kalanfa_plugin" == x.__module__
            )
        ...
        if "No module named '{}'".format(plugin_module_name) in exc_message:
            msg = (
                "Plugin '{}' exists but does not have an importable kalanfa_plugin module"
            ).format(plugin_name)
            raise PluginDoesNotExist(msg)
        else:
            raise
...
def enable_plugin(plugin_name, initialize_hooks=False):
    try:
        obj = initialize_kalanfa_plugin(plugin_name, initialize_hooks=initialize_hooks)
        if obj:
            obj.enable()
            return True
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    # just use the Kalanfa version and just run upgrades in line with
                    # Kalanfa instead.
                    return kalanfa.__version__
    else:
        return kalanfa.__version__
...
class PluginUpdateManager:
    ...
    def _update_plugin(self, plugin_name):
        ...
        from kalanfa.plugins.registry import registered_plugins
    ...
    if config["UPDATED_PLUGINS"]:
        ...
        if update_manager.errors:
            raise PluginUpdateException(
                "{} failed to update, please restart Kalanfa".format(
...
def autoremove_unavailable_plugins():
    ...
    """
    from kalanfa.plugins.registry import is_initialized
...
def check_plugin_config_file_location(version):
    if matches_version(version, "<0.13.0"):
        old_conf_file = os.path.join(KALANFA_HOME, "kalanfa_settings.json")
...
def iterate_plugins():
    ...
    eps = (
        all_entry_points.get("kalanfa.plugins", [])
        if isinstance(all_entry_points, dict)
        else all_entry_points.select(group="kalanfa.plugins")
    )
    for entry_point in eps:
        ...
        if name not in plugin_ids:
            plugin_ids.add(name)
            try:
                plugin = initialize_kalanfa_plugin(name, initialize_hooks=False)
                yield plugin
            except Exception:
                pass
