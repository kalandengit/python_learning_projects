...
from kalanfa.plugins.utils import is_external_plugin
...
def _validate_module_setting(
    ...
    if setting in existing_settings and setting not in _tuple_settings:
        raise ValueError(
            "Plugin settings should not be used to override Kalanfa default settings, use the Django settings module env var"
        )
...
def _apply_base_settings(plugin_instance, settings_module):
    ...
    # label in the Django App Registry with kalanfa core apps
    # or Kalanfa core plugins.
    app_config = AppConfig.create(plugin_instance.module_path)
    app_config.label = plugin_instance.module_path
    app_config.kalanfa_plugin = True
