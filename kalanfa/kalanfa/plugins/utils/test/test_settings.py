...
def _apply_base_settings():
    with patch("kalanfa.plugins.utils.settings._apply_base_settings") as apply_mock:
        yield apply_mock
...
def test_settings_module_validate_import_string(_apply_base_settings, django_settings):
    with patch("kalanfa.plugins.utils.settings.importlib") as import_mock:
        apply_settings("test")
...
def test_settings_raise_if_overwrite_base_setting(
    ...
    plugins = [plugin_mock]
    with patch("kalanfa.plugins.utils.settings.registered_plugins", plugins):
        with pytest.raises(ValueError):
...
def test_settings_warn_if_two_plugins_set_setting(
    ...
    with patch("kalanfa.plugins.utils.settings.registered_plugins", plugins):
        with patch("kalanfa.plugins.utils.settings.warnings") as warnings_mock:
            apply_settings(module_mock)
...
def test_settings_append_locale_path_external(django_settings):
    ...
    plugin_mock = MagicMock(settings_module=None, module_path="test")
    plugins = [plugin_mock]
    with patch("kalanfa.plugins.utils.settings.registered_plugins", plugins), patch(
        "kalanfa.plugins.utils.settings.i18n.get_installed_app_locale_path",
        return_value="test",
    ):
        apply_settings(module_mock)
...
def test_settings_not_append_locale_path_internal(django_settings):
    module_mock = ModuleType("module_mock")
    plugin_mock = MagicMock(settings_module=None, module_path="kalanfa.test")
    plugins = [plugin_mock]
    with patch("kalanfa.plugins.utils.settings.registered_plugins", plugins), patch(
        "kalanfa.plugins.utils.settings.i18n.get_installed_app_locale_path",
        return_value="test",
    ), patch("kalanfa.plugins.utils.settings.AppConfig"):
        apply_settings(module_mock)
