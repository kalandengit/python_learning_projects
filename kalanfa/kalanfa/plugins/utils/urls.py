...
def find_duplicate_slugs():
    ...
    for key, value in slugs.items():
        if len(value) > 1:
            if all(map(lambda x: x.startswith("kalanfa.plugins"), value)):
                # If we have multiple kalanfa core plugins setting the same url slug, we should error out.
                # This is not acceptable.
                raise RuntimeError(
                    "Multiple core kalanfa plugins define the same top level URL slug: {}".format(
                        key
                    )
                )
...
def get_urls():
    ...
    for plugin_instance in registered_plugins:
        ...
        # If we have a collision, and one of the plugins is a kalanfa core plugin, we let it take precedence.
        # If both are Kalanfa core plugins, we have raised a RuntimeError already when checking for
        # duplicate slugs.
