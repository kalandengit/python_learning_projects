...
<script>
  ...
  } from 'kalanfa-common/components/syncComponentSet/SelectDeviceModalGroup/useDevices';
  ...
  export default {
      ...
      const devicesById = computed(() => {
        return devices.value.reduce(
          ...
          },
          {
            [KDP_ID]: {
              device_id: KDP_ID,
              // eslint-disable-next-line kalanfa/vue-no-undefined-string-uses
    ...
    $trs: {
      ...
      introduction: {
        message:
          'Set a schedule for Kalanfa to automatically sync with other Kalanfa devices sharing this facility. Devices with the same sync schedule will be synced one at a time.',
