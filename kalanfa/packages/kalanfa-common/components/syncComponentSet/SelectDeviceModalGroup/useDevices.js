...
/**
 * Produces a function that resolves with a boolean if Kalanfa version is at least the specified
...
export function useDeviceMinimumVersionFilter(major, minor, patch) {
  const { isMinimumKalanfaVersion } = useMinimumKalanfaVersion(major, minor, patch);

  return async function deviceMinimumVersionFilter(device) {
    return isMinimumKalanfaVersion(device.kalanfa_version);
  };
}
