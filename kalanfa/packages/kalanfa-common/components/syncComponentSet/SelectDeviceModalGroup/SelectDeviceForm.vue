...
<script>
  ...
  export default {
    ...
    setup(props, context) {
      ...
      const storageDeviceId = useLocalStorage('kalanfa-lastSelectedNetworkLocationId', '');
      ...
      availableDeviceIds() {
        return this.devices
          .filter(
            device =>
              device.available &&
              (device.application === 'kalanfa' || this.$route.path === '/content'),
          )
    ...
    methods: {
      formatBaseDevice(device) {
        ...
        if (this.filterLODAvailable) {
          const version = device.kalanfa_version.split('.').slice(0, 3).join('.');
          return `${url}, Kalanfa ${version}`;
        } else return url;
      },
      resetSelectedDevice() {
    ...
    $trs: {
      ...
      lodSubHeader: {
        message:
          'Select a device with Kalanfa version 0.15 or greater to import learner user accounts',
        context:
