...
export const syncStrings = createTranslator('CommonSyncStrings', {
  ...
  howAreYouUsingKalanfa: {
    message: 'How are you using Kalanfa?',
...
export default {
  methods: {
    ...
    fetchNetworkLocationFacilities(locationId) {
      return client({
        url: urls['kalanfa:core:networklocation_facilities_detail'](locationId),
      })
        .then(response => {
    ...
    startKdpSyncTask(facility) {
      return TaskResource.startTask({ type: 'kalanfa.core.auth.tasks.dataportalsync', facility });
    },
