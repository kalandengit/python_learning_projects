 ...
 * @property {string} instance_id - Kalanfa instance UUID of the remote device
 ...
 * @property {string} application - application identifier (e.g. `kalanfa`)
 * @property {string} kalanfa_version - Kalanfa version string reported by the device
...
function updateConnectionStatus(id) {
  const url = urls['kalanfa:core:networklocation_update_connection_status'](id);
...
function fetchFacilities(id) {
  return this.client({
    url: urls['kalanfa:core:networklocation_facilities_detail'](id),
  }).then(response => {
    return response.data;
  });
}
