...
module.exports = function (outputDir, languageInfoPath) {
  ...
  *   https://kalanfa-dev.readthedocs.io/en/develop/i18n.html
  *
  * This file was generated by kalanfa-i18n code-gen
  *
  ...
  function generateVueIntlItems(language) {
    ...
    if (vue_intl_code !== 'ach') {
      ...
      try {
        ...
        // Fall back to kalanfa-i18n's bundled locale data - copy to polyfills
        try {
          const kalanfaPath = require.resolve(`kalanfa-i18n/src/locale-data/vue-intl/${filename}`);
          // Copy our polyfill to the output directory
          if (copyPolyfillFile(kalanfaPath, vueIntlPolyfillsDir, filename)) {
            module_path = `./polyfills/vue-intl/${filename}`;
          } else {
            throw new Error(`Failed to copy kalanfa-i18n vue-intl polyfill for ${vue_intl_code}`);
          }
        } catch (e) {
          logging.error(
            `${vue_intl_code} not available from vue-intl locale data and no kalanfa-i18n locale data found`,
          );
  ...
  function generateIntlItems(language) {
    ...
    if (language.intl_code !== 'ach-ug') {
      ...
      try {
        ...
        // Fall back to kalanfa-i18n's bundled locale data - copy to polyfills
        try {
          const kalanfaPath = require.resolve(`kalanfa-i18n/src/locale-data/intl/${jsFilename}`);
          // Copy our polyfill to the output directory
          if (copyPolyfillFile(kalanfaPath, intlPolyfillsDir, jsFilename)) {
            module_path = `./polyfills/intl/${jsFilename}`;
          } else {
            throw new Error(`Failed to copy kalanfa-i18n intl polyfill for ${language.intl_code}`);
          }
        } catch (e) {
          logging.error(
            `${language.intl_code} not available from intl locale data and no kalanfa-i18n locale data found`,
          );
