...
import { DisconnectionErrorCodes, SIGNED_OUT_DUE_TO_INACTIVITY } from 'kalanfa/constants';
import clientFactory from 'kalanfa/utils/baseClient';
import { browser, os, pageVisible } from 'kalanfa/utils/browserInfo';
...
// to prevent hammering the Kalanfa server when it is under load
...
// log in the Kalanfa server, if the user has been active in this time,
...
// sufficient to maintain the current Kalanfa session, but will start a
...
export class HeartBeat {
  ...
  monitorDisconnect(code = 0) {
    if (get(this._connection.connected)) {
      ...
      if (get(pageVisible)) {
        ...
        // Otherwise the issue is likely an overload of the Kalanfa server,
  ...
  _sessionUrl(id) {
    return urls['kalanfa:core:session_detail'](id);
  }
  /*
