...
describe('auth message component', () => {
  ...
  describe('tests for sign-in page link when user plugin exists', () => {
    beforeAll(() => {
      ...
      urls['kalanfa:kalanfa.plugins.user_auth:user_auth'] = userUrl;
      userUrl.mockReturnValue('http://localhost:8000/en/auth/');
    });

    afterAll(() => {
      delete urls['kalanfa:kalanfa.plugins.user_auth:user_auth'];
    });

    it('shows correct link text if there is a user plugin', () => {
      window.location.href = 'http://kalanfa.time/#/';
      const wrapper = makeWrapper();
      const link = wrapper.find('[data-testid=signinlink]');
      expect(link.attributes()).toMatchObject({
        href: 'http://localhost:8000/en/auth/#/signin?next=http%3A%2F%2Fkalanfa.time%2F%23%2F',
        text: 'Sign in to Kalanfa',
      });
    });
  });
