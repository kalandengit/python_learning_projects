...
<script>
  ...
  export default {
    ...
    computed: {
      rootUrl() {
        return urls['kalanfa:core:redirect_user']();
      },
      detailsText() {
      ...
      linkText() {
        if (!this.userAuthPluginUrl) {
          ...
          return this.$tr('signInToKalanfaAction');
        }
      },
      userAuthPluginUrl() {
    ...
    $trs: {
      admin: {
        ...
        context:
          'Message presented to any user *without* admin permissions who accidentally lands on a Kalanfa page that is reserved for admins.',
      },
      adminOrCoach: {
        message: 'You must be signed in as an admin or coach to view this page',
        context:
          'Message presented to any user *without* admin or coach permissions who accidentally lands on a Kalanfa page that is reserved for admins or coaches.',
      },
      learner: {
      ...
      registeredUser: {
        ...
        context:
          'Message presented to any user who attempts to view a page that requires them to be signed in to Kalanfa.',
      },
      superuser: {
        message: 'You must have super admin permissions to view this page',
        context:
          'Message presented to any user *without* super admin permissions who accidentally lands on a Kalanfa page that is reserved for super admins. ',
      },
      forgetToSignIn: {
        message: 'Did you forget to sign in?',
        context: 'Prompt to remind users to sign in.',
      },
      signInToKalanfaAction: {
        message: 'Sign in to Kalanfa',
        context:
          'This text appears as a prompt to sign in to Kalanfa if you already have an account and a username.\n\nIt may appear if your session has expired.',
      },
      goBackToHomeAction: {
        message: 'Go to home page',
        context:
          'Link that upon selecting takes the user back to the Kalanfa home page where the user can sign in.',
      },
      contentManager: {
        message:
          'You must be signed in as a super admin or have resource management permissions to view this page',
        context:
          'Message presented to any user *without* super admin or resource management permissions who accidentally lands on a Kalanfa page that is reserved for super admins.',
      },
    },
  };
