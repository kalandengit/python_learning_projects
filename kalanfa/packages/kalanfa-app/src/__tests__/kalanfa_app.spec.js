...
jest.mock(
  ...
  () => {
    return {
      registerKalanfaModuleSync: jest.fn(),
    };
  },
  { virtual: true },
);
...
class TestApp extends KalanfaApp {
  get pluginModule() {
    return {
      state() {
        return {
...
describe('KalanfaApp', function () {
  ...
  it('boots when pluginModule provides no state (Vuex is optional)', async function () {
    ...
    await expect(new KalanfaApp().ready()).resolves.toBeUndefined();
  });
