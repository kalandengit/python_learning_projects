...
const cliLogging = logger.getLogger('Kalanfa Build CLI');
const buildLogging = logger.getLogger('Kalanfa Build');
...
program.version(version.version).description('Build tools for Kalanfa frontend plugins');
...
function createWebpackCompiler(bundleData, options) {
  ...
  // Add timing hooks
  let start;
  compiler.hooks.run.tap('Kalanfa', () => {
    start = new Date();
  });
  compiler.hooks.watchRun.tap('Kalanfa', () => {
    start = new Date();
  });
  compiler.hooks.done.tap('Kalanfa', () => {
