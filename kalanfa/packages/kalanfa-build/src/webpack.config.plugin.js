 ...
 * @param {string} [options.kdsPath] - Path to a local Kalanfa Design System checkout to alias.
...
module.exports = (
  ...
  const isCoreBundle = webpackConfig.output && webpackConfig.output.library === kalanfaName;
  ...
  const alias = {};
  if (kdsPath) {
    alias['kalanfa-design-system'] = path.resolve(kdsPath);
    cache = false;
  }
  let bundle = {
    externals,
    ...
    plugins: [
      ...
      // they are given in kalanfa_plugins.py inside their relevant module.
      // Also define the current plugin version (for kalanfa plugins bundled with kalanfa, this is
      // the kalanfa version).
      // Also add the copyright year for auto updated copyright footers.
      new webpack.DefinePlugin({
        __kalanfaModuleName: JSON.stringify(data.name),
  ...
  if (isCoreBundle && mode === 'production') {
    bundle.plugins.push(
      // requires >= v3.0.0, which is specified in the kalanfa package
