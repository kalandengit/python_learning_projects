Kalanfa Sandbox: HTML5 App Bridge Library
========================================

About
-----

Kalanfa Sandbox is a library to allow for mocking of various HTML5 APIs inside a sandboxed iframe. In addition, it leverages the postMessage API to allow for controlled communication and data persistence from inside the sandboxed iframe. This means that HTML5 apps within Kalanfa can have persistent state that is backed by the ContentSummaryLog for that particular user.
...
Step 1: Install package deps (run from Kalanfa root)

`pnpm install`

Step 2: Build Kalanfa and kalanfa-sandbox

`pnpm run build`

Custom Navigation: Kalanfa Namespace Data Flow
-----------------------------------------------
...
External/partner product teams can create HTML5 applications that are fully embeddable within Kalanfa and can read Kalanfa content data, which they otherwise wouldn't be able to access.
...
The HTML5 app uses ``window.kalanfa.[function]`` within the app to access the API.

The API is defined in `kalanfa.js` and works through a promise chains, to manage async communication, and the postMessageAPI, to facilitate communication between current and parent windows.
...
`window.kalanfa.getContentById('id12345')`, as defined in `kalanfa.js`
...
The messages are managed through the function `sendMessageAwaitReply()` that is defined in `mediator.js`. It is used each time a ``window.kalanfa.[function]`` is called, and waits for the data to either be returned or for an error, before resolving and returning data to the original function.
...
In `mainClient.js`, there are listeners registered for all of the core event types for the kalanfa HTML5 API: data requested, navigate to, content, and data returned. On the event, the mediator then sends another message, that also contains the namespace, event, and message.
...
```
    this.on(this.events.DATAREQUESTED, message => {
      ...
      } else if (message.dataType === DataTypes.KALANFAVERSION) {
        event = events.KALANFAVERSIONREQUESTED;
      }
...
The listeners for "outgoing" messages are all in `CustomContentRenderer.vue`, the Vue component that renders the full screen view of the HTML5 App and manages requests to the kalanfa backend.
...
Here, when the event is omitted, the component uses existing kalanfa helper functions like `ContentNodeResource.fetchModel()` to request the data.
...
Finally, the same process of postMessages then happens in reverse, with `CustomContentRenderer.vue` sending a message to `mainClient.js`, which in turn sends a message to `mediator.js` which then resolves or rejects the promise that has been pending with `kalanfa.getContentById()`.
...
To regenerate, the repository should be cloned, and `pnpm run build` run within the context of that repository to regenerate the new assets. All the files put into `dist` should then be copied into `kalanfa/core/content/static/bloom` in the Kalanfa repository. Any previously existing hash named files can be deleted and replaced by the new hash named files.
