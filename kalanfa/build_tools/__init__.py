"""
This module is intended to allow easier build time configuration of Kalanfa.
...
All the environment variables should be prefixed with KALANFA_BUILD_ in the current environment,
...
BUILD_TIME_PLUGINS, a txt file containing an explicit list of all Kalanfa plugins that should
...
RUN_TIME_PLUGINS, a txt file containing an explicit list of all Kalanfa plugins that should be
enabled by default when Kalanfa runs. These plugins will be automatically enabled when Kalanfa
...
the base requirements for Kalanfa and bundled into the built dist folder. In the case of any
unbuilt external Kalanfa plugins, this should be a URL to a pip compatible source bundle.
