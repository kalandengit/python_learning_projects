Feature: Sync all to Kalanfa Data Portal
  When connected to the internet, users that are registered to a Project in Kalanfa Data Portal (KDP) should be able to sync all facilities on their device at one time
  ...
  Scenario: Try syncing all when there are no facilities that are registered to Kalanfa Data Portal
    Given I do not have a facility that is registered to Kalanfa Data Portal on my device
...
# Now register facility with KDP
  ...
  Scenario: Sync all to Kalanfa Data Portal is successful
    Given I am connected to the internet
      And at least one facility is registered to at least one Project on Kalanfa Data Portal
    When I click the *Sync all* button
    Then I see the *Sync all facility data* modal
    When I click the *Sync* button
    Then I see the list of facilities
      And I see a *Syncing* message under <X> facilities that are registered to a Project on Kalanfa Data Portal
