Feature: Learner sign-in
  ...
  Background:
    ...
    Scenario: Any subsequent session on Kalanfa after first sign in (multiple facilities; with password)
    ...
    Scenario: Any subsequent session on Kalanfa after first sign in (multiple facilities; no password)
      Given the device has more than one facility
        And I have used this device previously to sign in or create a new learner account
        And that signing in without password is disabled in the *Facility Settings*
      When I open Kalanfa in my browser
