Feature: Super admin customizes the Kalanfa settings with the options.ini file
    Super admin needs to be able to customize the Kalanfa settings by modifying the options.ini file
  ...
  Scenario: Change the default HTTP port
    When I add "HTTP_PORT = <port number>" under the [Deployment] section
      ...
      And I run the "kalanfa start" command in the terminal or command prompt
    Then I see that the Kalanfa server is running at <port number> port

  Scenario: Configure Kalanfa to display a specific set of languages
    ...
    Then I see only the specified languages in the language selector modal window
      And I can select a language and see the Kalanfa UI displayed in the selected language

  Scenario: Change the default Content directory
    When I add "CONTENT_DIR = <content folder>" under the [Paths] section
      And I save my changes
      And I run the "kalanfa manage content movedirectory <destination>" command in the terminal or command prompt
  ...
  Scenario: Change the default central content base URL
    ...
    Then I see the *Select a source* modal
      And I see the *Kalanfa Studio (online)* option selected
  ...
  Scenario: Change the default Data portal syncing base URL
    Given my facility is registered to a Project on Kalanfa Data Portal
...
Examples:
  ...
  | 8081        | en                 | C:\Users\IEUser\.kalanfa\content | C:\Users\IEUser\.kalanfa\| https://hotfixes.studio.learningequality.org/ |
