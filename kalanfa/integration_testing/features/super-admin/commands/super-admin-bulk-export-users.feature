Feature: Super Admin export users for a facility in a device with manage bulkexportusers command
  ...
  Background:
    Given that the Kalanfa server has one device with one or more facilities
      And Kalanfa is already localized in <language> language

  Scenario: Execute the bulkexportusers command and review the created 'users_<date>_<time>.csv' file
    When I run the 'kalanfa manage bulkexportusers --overwrite --output-file=test.csv' command in the Terminal
  ...
  Scenario: Execute the bulkexportusers command and review the header in the output file is translated
    When I run the 'kalanfa manage bulkexportusers --overwrite --output-file=test.csv --locale=es_ES' command in the Terminal
      And I browse the folder where I executed the command
