Feature: Admin registers facility
  Admin needs to be able to register their facility to Kalanfa Data Portal

  Background:
    Given I am logged in as a Facility admin
      And I have access to a Project token on Kalanfa Data Portal
      And I am at *Facility > Data*

  Scenario: Register to a Kalanfa Data Portal project for the first time
    Given my facility has never been registered
    When I click *Sync*
    Then I see the *Select a source* modal
    	And I see that *Kalanfa Data Portal (online)* is the selected option
    When I click the *Continue* button
    Then I see the *Register facility* modal
    When I enter the project token from Kalanfa Data Portal
  ...
  Scenario: Register to a second Kalanfa Data Portal project
    Given my facility has been registered before
      And I have access to a different Kalanfa Data Portal project token
  ...
  Scenario: Registration to a Kalanfa Data Portal project failed
    Given I have an invalid project token from Kalanfa Data Portal
    When I click the *Options* button next to the *Sync* button
    	And I click the *Register* option
    Then I see the *Register facility* modal
    When I enter the invalid project token from Kalanfa Data Portal
    	And I click *Continue*
    Then I see an *Invalid token* error message

  Scenario: Register to a Kalanfa Data Portal project, but already registered
    Given my facility has been registered before
    When I click *Register*
