Feature: Super Admin runs manage profile command
    Super Admin needs to be able to generate and review profile logs on the device running Kalanfa

  Background:
    Given that the Kalanfa server is running
      And the '[Server]' section of the Kalanfa's "options.ini" file is configured with 'PROFILE = 1' to enable request profiling

  Scenario: Execute the profile command and review the created "_performance.csv" file
    When I run the 'kalanfa manage profile --num-samples=6' command in the Terminal
      And I browse the "KALANFA_HOME/performance folder"      # Usually $HOME/.kalanfa/performance
  ...
  Scenario: Add more interactions and review the created "_requests_performance.csv" file
    When I interact with Kalanfa in the browser by navigating through other server pages for at least 1 minute
      And I browse the "KALANFA_HOME/performance" folder again
  ...
  Scenario: Stop interacting with server and observe profiling results
    ...
    Then I open CSV files in a text editor or a spreadsheet application
      ...
      And I see that "xxxxxxxx_xxxxxx_requests_performance.csv" file has at least one line per each Kalanfa page I interacted with in the browser, and lines begin with a timestamp followed by requests profiling results
