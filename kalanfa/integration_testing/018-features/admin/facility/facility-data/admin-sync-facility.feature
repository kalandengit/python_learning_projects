Feature: Admin syncs facility
  Admin needs to be able to sync their facility data to Kalanfa Data Portal or another Kalanfa server on their local network or internet

  Background:
    Given I am signed in as a Facility admin
    	And I am at *Facility > Data*
      And my facility is already registered
      And there are other devices with Kalanfa on the network

  Scenario: Learn what sync does
    When I click on *Usage and privacy* in the description of the *Sync facility data* section
    Then I see the *Kalanfa data portal* modal with a description of what syncing does
  ...
  Scenario: Successful sync to KDP
    ...
    Then I see the *Select a source* modal window
      And I see a *Kalanfa Data Portal (online)* radio button (selected by default)
  ...
  Scenario: Failed sync to KDP
    When I modify the options.ini file so that the variable *KALANFA_DATA_PORTAL_SYNCING_BASE_URL* is set to a fake URL
    	And I click *Sync*
    Then I see the *Select a source* modal window
      And I see a *Kalanfa Data Portal (online)* radio button (selected by default)
      And I see a *Local network or internet* radio button
