...
The Kalanfa server represents data as Django Models. These models are defined in ``models.py`` files, which can be found in the folders of the different Django apps/plugins.

In Django, Model data are usually exposed to users through webpages that are generated by the Django server. To make the data available to the Kalanfa client, which is a single-page app, the Models are exposed as JSON data through a REST API provided by the Django REST Framework (DRF). It's important to remark that Kalanfa limits the content types the DRF api support to be only ``application/json`` or ``multipart/form-data``. This limitation is set at :code:`kalanfa/core/negotiation.py`.
...
To explore the server REST APIs, visit `/api_explorer/` on the Kalanfa server while running with developer settings.
...
If this resource is part of the core app, it can be added to a global registry of resources inside :code:`packages/kalanfa-common/apiResources`. Otherwise, it can be imported as needed, such as in the coach reports module.
