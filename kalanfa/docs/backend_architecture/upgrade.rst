...
Kalanfa can be automatically upgraded forwards. For instance, you can upgrade
...
these migrations can be generated by executing ``kalanfa manage makemigrations``.
...
and replacing the reference to `settings.AUTH_USER_MODEL` with `"kalanfaauth.FacilityUser"`
instead. For an example of this, see this previous pull request: https://github.com/learningequality/kalanfa/pull/11984 .
...
``cli.py`` - those commands that are instantiated using the ``cls=KalanfaDjangoCommand``
...
are put into Kalanfa in order to facilitate moving between versions. This may be
for bug fixing or efficiency purposes. These are sometimes carried out outside of
migrations in order to leverage the full Kalanfa code base, which can be restricted
inside the contexts of Django data migrations.

In order to implement these upgrades, a decorator is available in ``kalanfa.core.upgrade``,
...
Django setting, or is in an activated Kalanfa plugin, this upgrade will be picked up and run any time
an upgrade happens from a Kalanfa version older than ``0.6.4`` to a Kalanfa version equal to or newer
than ``1.0.0``.
