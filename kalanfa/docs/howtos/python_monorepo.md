...
Kalanfa's Python code is organized as a [uv workspace](https://docs.astral.sh/uv/concepts/projects/workspaces/). The main `kalanfa` package is the workspace root; additional packages live under `python_packages/`.
...
2. If it depends on `kalanfa` itself, resolve that dependency against this workspace instead of PyPI:
   ```toml
   [tool.uv.sources]
   kalanfa = { workspace = true }
   ```
...
5. Run `uv sync --group dev --all-packages` to install the new package into the shared workspace venv. `--all-packages` is required — a plain `uv sync` (or `uv sync --package <package-name>`) scopes the sync to a single project and drops root Kalanfa's own runtime dependencies (Django, Click, etc.) from the shared venv.
...
Member package versions are independent of each other and of the main `kalanfa` package — there's no enforcement linking them. Use a static `version = "x.y.z"` field, not `setuptools-scm`-derived dynamic versioning: this repo's git tags are Kalanfa's own release tags, so dynamic versioning inside the workspace would report Kalanfa's version instead of the package's own.
...
- **Stage 1** (blocking, fast feedback): core Kalanfa tests on Python 3.10, and Postgres tests. Runs in parallel.
