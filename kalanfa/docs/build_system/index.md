...
- **kalanfa-app**: macOS .dmg with app signing and notarization
- **kalanfa-installer-debian**: .deb packages for Debian/Ubuntu
- **kalanfa-installer-windows**: .exe with Windows code signing
- **kalanfa-installer-android**: .apk with Google Play Store publishing
...
3. **Platform Builds**: Triggers platform-specific build workflows:
   - **DMG** (macOS disk image): `learningequality/kalanfa-app`
   - **DEB** (Debian/Ubuntu package): `learningequality/kalanfa-installer-debian`
   - **EXE** (Windows installer): `learningequality/kalanfa-installer-windows`
   - **APK** (Android app package): `learningequality/kalanfa-installer-android`
...
```yaml
dmg:
  uses: learningequality/kalanfa-app/.github/workflows/build_mac.yml@v0.4.4
  with:
...
2. `pr_build_kalanfa.yml` triggers automatically
...
2. `release_kalanfa.yml` triggers automatically
