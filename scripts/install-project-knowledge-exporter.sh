#!/usr/bin/env bash
#
# install-project-knowledge-exporter.sh
# -------------------------------------
# Run this from anywhere inside a git repo (or drop it into a repo root) to
# install/update the `project-knowledge-exporter` Claude Code skill:
#
#   1. Writes  .claude/skills/project-knowledge-exporter/SKILL.md
#   2. Adds a marked "project-knowledge-exporter" section to CLAUDE.md (creating
...
#
# Usage:
#   ./install-project-knowledge-exporter.sh                 # install files only
#   COMMIT=1 ./install-project-knowledge-exporter.sh        # + commit on a branch
#   COMMIT=1 PUSH=1 ./install-project-knowledge-exporter.sh # + push the branch
#
set -euo pipefail

# ---- config (override via environment) --------------------------------------
BRANCH="${BRANCH:-add-project-knowledge-exporter-skill}"
COMMIT="${COMMIT:-0}"   # 1 = git add + commit on a new branch
PUSH="${PUSH:-0}"       # 1 = git push -u origin <branch> (implies COMMIT)
RAW_URL="${RAW_URL:-https://raw.githubusercontent.com/kalandengit/python_learning_projects/main/skills/project-knowledge-exporter/SKILL.md}"
# -----------------------------------------------------------------------------
...
SKILL_DIR=".claude/skills/project-knowledge-exporter"
SKILL_FILE="$SKILL_DIR/SKILL.md"
CLAUDE_FILE="CLAUDE.md"
BEGIN="<!-- BEGIN project-knowledge-exporter (managed) -->"
END="<!-- END project-knowledge-exporter (managed) -->"
...
if [ "$COMMIT" = "1" ]; then
  ...
  if git diff --cached --quiet; then
    ...
    git commit -m "Add/update project-knowledge-exporter Claude Code skill"
