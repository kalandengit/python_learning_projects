...
description: Analyze an entire software project and generate a durable, LLM-portable Project Knowledge Package (blueprint, structured context JSON, rebuild prompt, decision log, and development-state report) so the project can be understood, maintained, extended, and reconstructed even if the original repo, git history, and conversations are lost. Use when the user asks to export project knowledge, document a codebase end-to-end, produce a rebuild/handoff package, preserve architecture and engineering decisions, or generate an LLM-ingestible snapshot of a project.
when_to_use: The user wants to preserve, hand off, or reconstruct a project — phrases like "export project knowledge", "document this whole project", "make a rebuild/handoff package", "onboard another LLM/team", "preserve the architecture and decisions", "generate PROJECT_BLUEPRINT / PROJECT_CONTEXT / rebuild prompt", or "snapshot this codebase for the future".
version: 1.0.0
author: kalandengit
---
