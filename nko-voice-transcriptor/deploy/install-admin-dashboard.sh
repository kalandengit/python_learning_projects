#!/usr/bin/env bash
# Install a tiny authenticated dashboard at https://DOMAIN/server-admin/
set -Eeuo pipefail

DOMAIN="${DOMAIN:-nko.kalandeh.kalanfa.org}"
...
NGINX_SITE="${NGINX_SITE:-/etc/nginx/sites-available/nko-transcriptor}"
...
grep -q "listen 443 ssl" "$NGINX_SITE" || die "HTTPS is not configured in $NGINX_SITE"
...
apt-get update -qq
apt-get install -y -qq python3 nginx apache2-utils curl sudo
...
cat > /etc/nginx/snippets/nko-admin-dashboard.conf <<EOF
...
import pathlib, sys
path, domain = pathlib.Path(sys.argv[1]), sys.argv[2]
text = path.read_text()
include = "    include /etc/nginx/snippets/nko-admin-dashboard.conf;\n"
if include.strip() in text:
    raise SystemExit
start = 0
while True:
    start = text.find("server {", start)
    if start < 0: raise SystemExit("HTTPS server block not found")
    depth, pos = 0, start
    while pos < len(text):
        if text[pos] == "{": depth += 1
        elif text[pos] == "}":
    ...
    if domain in block and "listen 443 ssl" in block:
...
echo
echo "SUCCESS: https://$DOMAIN/server-admin/"
