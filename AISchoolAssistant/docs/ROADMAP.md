...
| **WP-4** | **Eventing + Knowledge Platform** | `@asa/eventing`: DomainEvent envelope + Event Catalog (schema-validated) + EventBus port/InMemoryEventBus + catalog-validated EventPublisher + observability (ADR-0006). `@asa/knowledge`: VectorStore port/InMemoryVectorStore (cosine) + embedding-backed KnowledgeService, tenant-scoped (ADR-0007). Service template: Learner Digital Twin as an event-sourced projection + knowledge ingest/search, with e2e. NATS JetStream + Qdrant/OpenSearch adapters follow. | **done** |
